...
{
    public partial class InternalIBPSLReport : CENTRUMBase
    {
        ...
        private void InitBasePage()
        {
            ...
            {
                this.Menu = false;
                this.PageHeading = "Internal IBPSL Report";
                this.ShowBranchName = Session[gblValue.BrnchCode].ToString() + " - " + Session[gblValue.BrName].ToString();
                this.ShowFinYear = Session[gblValue.FinYear].ToString() + " ( Login Date " + Session[gblValue.LoginDate].ToString() + " )";
                this.GetModuleByRole(mnuID.mnuRptIBPSL);
                ...
                {
                    Response.Redirect("~/WebPages/Public/PageAccess.aspx?mnuTxt=" + "Internal IBPSL Report", false);
                }
            }
            catch
            {
        ...
        private void SetRptData(string pMode)
        {
            ...
            {
                oRpt = new CReports();
                //dt = oRpt.RptIBPSL(vAsOnDt, vBrCode);

                var req = new IBPSLReportRequest()
                {
                ...
                Int32 vRptIBPSL = Convert.ToInt32(Session[gblValue.RptIBPSL].ToString());
                if (vRptIBPSL != 0)
                {
                    Int32 unixTicks = (Int32)(DateTime.UtcNow.Subtract(new DateTime(1970, 1, 1))).TotalSeconds;
                    if (unixTicks - vRptIBPSL > 300)
                    {
                        Session[gblValue.RptIBPSL] = ((Int32)(DateTime.UtcNow.Subtract(new DateTime(1970, 1, 1))).TotalSeconds).ToString();
                        GenerateReport("GenerateIBPSL", Requestdata);
                    }
                    else
                    {
    ...
    [DataContract]
    public class IBPSLReportRequest
    {
        [DataMember]
