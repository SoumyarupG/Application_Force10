...
{
    public partial class IncomeItemMaster : CENTRUMBAse
    {
        ...
        private void InitBasePage()
        {
            try
            {
                ...
                this.Menu = false;
                this.PageHeading = "Income Item  Master";
                this.ShowBranchName = Session[gblValue.BrnchCode].ToString() + " - " + Session[gblValue.BrName].ToString();
                this.ShowFinYear = Session[gblValue.FinYear].ToString() + " ( Login Date " + Session[gblValue.LoginDate].ToString() + " )";
                this.GetModuleByRole(mnuID.mnuIncomeItemMst);
        ...
        protected void gvDist_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            try
            {
                ...
                if (e.CommandName == "cmdShow")
                {
                    ...
                    dt = oDist.GetIncomeItemById(pId);
                    if (dt.Rows.Count > 0)
                    {
                        hfItemId.Value = Convert.ToString(dt.Rows[0]["Id"]);
                        txtIncomeItem.Text = Convert.ToString(dt.Rows[0]["Product"]);
        ...
        private Boolean SaveRecords(string Mode)
        {
            ...
            try
            {
                ...
                if (Mode == "Save")
                {
                    ...
                    dtCount = oGbl.ChkIncomeItemDuplicate(txtIncomeItem.Text.Replace("'", "''"), vItemId, "Save");
                    ...
                    vErr = oDist.SaveIncomeItem(ref vNewId, vItemId, vOccSubTypeId, txtIncomeItem.Text.Replace("'", "''"), this.UserID, "Save");
                    if (vErr > 0)
                    {
                        ViewState["Id"] = vNewId;
                        vResult = true;
                ...
                else if (Mode == "Edit")
                {
                    ...
                    oGbl = new CGblIdGenerator();
                    DataTable dtCount = new DataTable();
                    dtCount = oGbl.ChkIncomeItemDuplicate(txtIncomeItem.Text.Replace("'", "''"), vItemId, "Edit");
                    if (dtCount.Rows.Count > 0)
                    {
                        if (Convert.ToInt32(dtCount.Rows[0]["NoOfRecord"].ToString()) > 0)
                        {
                    ...
                    vErr = oDist.SaveIncomeItem(ref vNewId, vItemId, vOccSubTypeId, txtIncomeItem.Text.Replace("'", "''"), this.UserID, "Edit");
                    if (vErr > 0)
                    {
                ...
                else if (Mode == "Delet")
                {
                    oDist = new CDistrict();
                    oGbl = new CGblIdGenerator();
                    // vRec = oGbl.ChkIncomeItemDelete(vItemId, "DistrictID", "VillageMst");
                    DataTable dtCount = new DataTable();
                    //dtCount = oGbl.ChkIncomeItemDelete(txtIncomeItem.Text.Replace("'", "''"), vItemId, "Delet");
                    dtCount = oGbl.ChkIncomeItemDelete(vItemId);
                    if (dtCount.Rows.Count > 0)
                    {
                        if (Convert.ToInt32(dtCount.Rows[0]["CountProduct"].ToString()) > 0)
                        {
                            gblFuction.MsgPopup("Item cannot be deleted");
                            return false;
                        }
                    }
                    vErr = oDist.SaveIncomeItem(ref vNewId, vItemId, vOccSubTypeId, txtIncomeItem.Text.Replace("'", "''"), this.UserID, "Delet");
                    if (vErr > 0)
                    {
                        gblFuction.MsgPopup(gblPRATAM.DeleteMsg);
                        //ViewState["Id"] = vNewId;
