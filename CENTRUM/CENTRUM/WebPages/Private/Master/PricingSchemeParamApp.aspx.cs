...
{
    public partial class PricingSchemeParamApp : CENTRUMBase
    {
        ...
        private void InitBasePage()
        {
            try
            {
                ...
                this.Menu = false;
                this.PageHeading = "Pricing Scheme Parameter Approval";
                this.ShowBranchName = Session[gblValue.BrnchCode].ToString() + " - " + Session[gblValue.BrName].ToString();
                this.ShowFinYear = Session[gblValue.FinYear].ToString() + " ( Login Date " + Session[gblValue.LoginDate].ToString() + " )";
                this.GetModuleByRole(mnuID.mnuPricingSchemeParameterApp);
                if (Convert.ToInt32(Session[gblValue.RoleId]) == 1) return;
                if (this.CanView == "Y" && this.CanAdd != "Y" && this.CanProcess != "Y")
                {
                    btnApprove.Visible = false;
                    btnReject.Visible = false;
                    btnCancel.Visible = false;
                }
                else if (this.CanView == "Y" && this.CanAdd == "Y" && this.CanProcess != "Y")
                {
                ...
                {
                    Response.Redirect("~/WebPages/Public/PageAccess.aspx?mnuTxt=" + "Pricing Scheme Parameter Approval", false);
                }
            }
            catch
            {
        ...
        private void LoadGrid(Int32 pPgIndx)
        {
            ...
            try
            {
                ...
                dt = oIC.GetPricingSchemeAppPG(pPgIndx, ref vRows);
        ...
        private Boolean SaveRecords(string Mode)
        {
            ...
            try
            {
                ...
                if (Mode == "App")
                {
                    ...
                    if (vErr > 0)
                    {
                        gblFuction.MsgPopup("Successfully Approved");
                        vResult = true;
                    }
                    else
                    {
                ...
                else if (Mode == "Rej")
                {
                    ...
                    if (vErr > 0)
                    {
                        gblFuction.MsgPopup("Successfully Rejected");
                        vResult = true;
                    }
                    else
                    {
