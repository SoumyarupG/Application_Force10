...
{
    public partial class NEFTDisburseApprovalHO : CENTRUMBase
    {
        ...
        private void InitBasePage()
        {
            ...
            {
                this.Menu = false;
                this.PageHeading = "NEFT Disbursement Maker";
                this.ShowBranchName = Session[gblValue.BrnchCode].ToString() + " - " + Session[gblValue.BrName].ToString();
                this.ShowFinYear = Session[gblValue.FinYear].ToString() + " ( Login Date " + Session[gblValue.LoginDate].ToString() + " )";
                this.GetModuleByRole(mnuID.mnuNEFTDisbApprovalHO);
                ...
                {
                    Response.Redirect("~/WebPages/Public/PageAccess.aspx?mnuTxt=" + "NEFT disbursement Approval", false);
                }
            }
            catch
            {
        ...
        private void LoadGrid(Int32 pPgIndx)
        {
            ...
            try
            {
                ...
                dt = oNeft.GetNEFTDisbApproval(gblFuction.setDate(txtDt.Text), ddlBranch.SelectedValues.Replace("|", ","));
        ...
        private Boolean SaveRecords(string Mode)
        {
            ...
            try
            {
                if (Mode == "Save")
                {
                    ...
                    //    return false;
                    //}
                    vErr = oNEFT.InsertNEFTDisbApproval(vXml, this.UserID, gblFuction.setDate(txtDt.Text), "I", Convert.ToDouble(txtTotNEFT.Text));
                    if (vErr == 5)
                    {
                            gblFuction.AjxMsgPopup("One member in the List is having one open... Loan cannot proceed");
                            vResult = false;
                    }
                    else
                    {
        ...
        protected void gvDtl_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                //TextBox txtRemark = (TextBox)e.Row.FindControl("Remarks");
                try
                {
                    ...
                    //txtRemark.Text = e.Row.Cells[37].Text.Trim();
                    if (e.Row.Cells[34].Text == "1")
        ...
        private void GenerateReport()
        {
            ...
            htw.WriteLine("<tr><td align=left' colspan='12'><font size='3'>from our Current Account No:  " + dt2.Rows[0]["Phone"].ToString() + " to the below mentioned accounts of Unity Small Finance Bank Limited.</font></td></tr>");
            htw.WriteLine("<tr><td colspan='12'><font size='3'></font></td></tr>");
        ...
        {
            DataTable dt = null;
            dt = (DataTable)ViewState["NEFT"];
            TextBox textbox = (TextBox)sender;
            GridViewRow row = (GridViewRow)textbox.NamingContainer;
            TextBox txtRemark = (TextBox)row.FindControl("txtRemark");
            dt.Rows[row.RowIndex]["Remarks"] = txtRemark.Text;
            dt.AcceptChanges();
             
            
        }
        protected void txtCashRemark_textchanged(object sender, EventArgs e)
        {
            DataTable dt = null;
            dt = (DataTable)ViewState["NEFT"];
            TextBox textbox = (TextBox)sender;
            GridViewRow row = (GridViewRow)textbox.NamingContainer;
            TextBox txtCashRemark = (TextBox)row.FindControl("txtCashRemark");
            dt.Rows[row.RowIndex]["CashRemarks"] = txtCashRemark.Text;
            dt.AcceptChanges();


        }
        protected void txtPostRemark_textchanged(object sender, EventArgs e)
        {
            DataTable dt = null;
            dt = (DataTable)ViewState["NEFT"];
            TextBox textbox = (TextBox)sender;
            GridViewRow row = (GridViewRow)textbox.NamingContainer;
            TextBox txtPostRemark = (TextBox)row.FindControl("txtPostRemark");
            dt.Rows[row.RowIndex]["PostRemarks"] = txtPostRemark.Text;
            dt.AcceptChanges();
