...
{
    public partial class rptRunDown : CENTRUMBase
    {
        ...
        private void InitBasePage()
        {
            ...
            {
                this.Menu = false;
                this.PageHeading = "RUN Down Report";
                this.ShowBranchName = Session[gblValue.BrnchCode].ToString() + " - " + Session[gblValue.BrName].ToString();
                this.ShowFinYear = Session[gblValue.FinYear].ToString() + " ( Login Date " + Session[gblValue.LoginDate].ToString() + " )";
                this.GetModuleByRole(mnuID.mnuRunDwnRpt);
        ...
        protected void btnExcl_Click(object sender, EventArgs e)
        {
           ...
           // SetRptData("Excel");
            Int32 vRptRunDown = Convert.ToInt32(Session[gblValue.RptRunDown].ToString());
            if (vRptRunDown != 0)
            {
                Int32 unixTicks = (Int32)(DateTime.UtcNow.Subtract(new DateTime(1970, 1, 1))).TotalSeconds;
                if (unixTicks - vRptRunDown > 300)
                {
                    Session[gblValue.RptRunDown] = ((Int32)(DateTime.UtcNow.Subtract(new DateTime(1970, 1, 1))).TotalSeconds).ToString();
                    GenerateRptData("Excel");
                }
                else
                {
        ...
        private void SetRptData(string pMode)
        {
            ...
            try
            {
                ...
                {
                    dt = oRpt.rptRunDown(vAsOnDt, vBrCode);
                }
                else
                {
                    dt = oRpt.rptRunDown_MEL(vAsOnDt, vBrCode);
                }

                if (pMode == "Excel")
                {
                    string vFileNm = "attachment;filename=" + txtAsOnDt.Text + "_RunDown.xls";
                    ...
                    if (ddlProject.SelectedItem.Value == "J")
                    {
                        Response.Write("<tr><td align=center' colspan='" + dt.Columns.Count + "'><b><font size='3'> RUN DOWN - JLG </font></b></td></tr>");
                    }
                    {
                        Response.Write("<tr><td align=center' colspan='" + dt.Columns.Count + "'><b><font size='3'> RUN DOWN - MEL </font></b></td></tr>");
                    }
        ...
        {
            string vBrCode = Convert.ToString(ViewState["BrCode"]);
            var req = new RunDownReportRequest()
            {
                pAsOnDt = txtAsOnDt.Text,
                pBrCode = vBrCode,
                pProject=ddlProject.SelectedItem.Value,
                pFormat = pMode,
            ...
            string vMsg = API.GenerateReport("GenerateRunDown", Requestdata, vReportUrl);
        ...
        private void PrintTxt(DataTable dt)
        {
            ...
            string vFileNm = "";
            vFileNm = vFolderPath + "\\RunDownReport_Centrum.txt";
    ...
    [DataContract]
    public class RunDownReportRequest
    {
        [DataMember]
