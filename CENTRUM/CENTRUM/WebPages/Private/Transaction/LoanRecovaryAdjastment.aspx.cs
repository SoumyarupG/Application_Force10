    ...
    public partial class LoanRecovaryAdjastment : CENTRUMBase
    {
        ...
        private void InitBasePage()
        {
            try
            {
                ...
                btnAdd.Visible = this.CanAdd == "Y" ? true : false;
                //btnEdit.Visible = Convert.ToBoolean(this.CanEdit);
                btnDelete.Visible = this.CanDelete == "Y" ? true : false;
        ...
        protected void btnAdd_Click(object sender, EventArgs e)
        {
            try
            {
                txtTranDt.Text = Session[gblValue.LoginDate].ToString();
                PopRO();
                if ((Convert.ToInt32(Session[gblValue.UserId]) != 1) && (Convert.ToBoolean((this.CanAdd == "") ? "false" : this.CanAdd == "Y" ? "true" : "false") == false))
                {
