...
{
    public partial class CF_OtherRep : CENTRUMBAse
    {
        ...
        private void InitBasePage()
        {
            try
            {
                ...
                this.Menu = false;
                this.PageHeading = "Other Reports";
                this.ShowBranchName = Session[gblValue.BrnchCode].ToString() + " - " + Session[gblValue.BrName].ToString();
                this.ShowFinYear = Session[gblValue.FinYear].ToString() + " ( Login Date " + Session[gblValue.LoginDate].ToString() + " )";
                this.GetModuleByRole(mnuID.mnuCFOthrRep);
                ...
                {
                    Response.Redirect("~/WebPages/Public/PageAccess.aspx?mnuTxt=" + "Other Reports", false);
                }
            }
            catch
            {
        ...
        private void ShowEmpRecords()
        {
            ...
            try
            {
                ...
                dt = oMem.CF_OthrRepDtlByLeadId(Convert.ToInt64(vLoanAppId));
                if (dt.Rows.Count > 0)
                {
                    gvEmpDoc.DataSource = dt;
                    gvEmpDoc.DataBind();
        ...
        {            
            ...
            if (GetDoc() == true)
            {
                ...
                vErr = oMem.CF_SaveOtherRepDtl(vLeadID, gblFuction.setDate(Session[gblValue.LoginDate].ToString()), vXmlData, DocumentBucketURL, Mode, this.UserID);
                if (vErr == 0)
                {
                    if (Mode == "Save")
                        gblFuction.MsgPopup(gblPRATAM.SaveMsg);
        ...
        #endregion
        protected void GetOtherRepDtlByLeadID()
        {
            Int32 pLeadId = 0;
            string vBrCode = "";
            ...
            try
            {
                if (Session[gblValue.LeadID] != null)
                {
                    pLeadId = Convert.ToInt32(Session[gblValue.LeadID]);
                }
