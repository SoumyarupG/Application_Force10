...
{
    public partial class CAMBucket : CENTRUMBAse
    {
        ...
        private void InitBasePage()
        {
            try
            {
                ...
                this.Menu = false;
                this.PageHeading = "CAM Bucket";
                this.ShowBranchName = Session[gblValue.BrnchCode].ToString() + " - " + Session[gblValue.BrName].ToString();
                this.ShowFinYear = Session[gblValue.FinYear].ToString() + " ( Login Date " + Session[gblValue.LoginDate].ToString() + " )";
                this.GetModuleByRole(mnuID.mnuCAMBucket);
                ...
                {
                    Response.Redirect("~/WebPages/Public/PageAccess.aspx?mnuTxt=" + "CAM Bucket", false);
                }
            }
            catch
            {
        ...
        private void LoadPendingCAMList()
        {
            DataSet ds = new DataSet();
            DataTable dt1 = new DataTable();
            DataTable dt = new DataTable();
            ...
            try
            {
                ds = oMem.GetPendingCAMList(txtSearch.Text.Trim(), vBrCode);
                if (ds.Tables.Count > 0)
                {
                    dt1 = ds.Tables[0];
                    if (dt1.Rows.Count > 0)
        ...
        private Boolean SaveCAMRecords(string Mode)
        {
            ...
            try
            {
                ...
                CMember OMem = new CMember();
                dtCustId = OMem.GetCustIdByLnAppId(vAppId);
                if (dtCustId.Rows.Count > 0)
                {
                    pCustId = dtCustId.Rows[0]["CustID"].ToString();
                }
                else
                {
                    gblFuction.AjxMsgPopup("Customer Not Found...");
                    return false;
                }
