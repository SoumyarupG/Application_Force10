...
{
    public partial class BCLoanDisburse : CENTRUMBase
    {
        ...
        private void InitBasePage()
        {
            ...
            {
                this.Menu = false;
                this.PageHeading = "BC Loan Disbursement";
                this.ShowBranchName = Session[gblValue.BrnchCode].ToString() + " - " + Session[gblValue.BrName].ToString();
                this.ShowFinYear = Session[gblValue.FinYear].ToString() + " ( Login Date " + Session[gblValue.LoginDate].ToString() + " )";
                this.GetModuleByRole(mnuID.mnuBCLoanDisb);
                ...
                {
                    Response.Redirect("~/WebPages/Public/PageAccess.aspx?mnuTxt=" + "BC Loan Disbursement", false);
                }
            }
            catch
            {
        ...
        private void SetRptData(string pMode)
        {
            ...
            try
            {
                ...
                dt = oRpt.rptBCLoanDisb(vAsOnDt, vBrCode);
                if (pMode == "Excel")
                {
                    string vFileNm = "attachment;filename=" + txtAsOnDt.Text + "_BCLoanDisb.xls";
                    ...
                    Response.Write("<tr><td align=center' colspan='" + dt.Columns.Count + "'><b><font size='3'> BC Loan Disbursement </font></b></td></tr>");
                    ...
                    foreach (DataRow dtrow in dt.Rows)
                    {
                        ...
                        for (int j = 0; j < dt.Columns.Count; j++)
                        {
                            ...
                            {
                                if (dtrow["COAPPLICANT1ID"].ToString().Length == 12)
                                {
                                    dtrow["COAPPLICANT1ID"] = String.Format("{0}{1}", "********", Convert.ToString(dtrow["COAPPLICANT1ID"]).Substring(Convert.ToString(dtrow["COAPPLICANT1ID"]).Length - 4, 4));
                                }
                                if (dtrow["COAPPLICANT2ID"].ToString().Length == 12)
                                {
                                    dtrow["COAPPLICANT2ID"] = String.Format("{0}{1}", "********", Convert.ToString(dtrow["COAPPLICANT2ID"]).Substring(Convert.ToString(dtrow["COAPPLICANT2ID"]).Length - 4, 4));
                                }
                            }

                            if (dt.Columns[j].ColumnName == "COAPPLICANT1ID" || dt.Columns[j].ColumnName == "COAPPLICANT2ID")
                            {
        ...
        private void PrintTxt(DataTable dt)
        {
            ...
            string vFileNm = "";
            vFileNm = vFolderPath + "\\BCLoanDisb.txt";
