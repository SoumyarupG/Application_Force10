...
{
    public partial class rptFunder : CENTRUMBase
    {
        ...
        private void InitBasePage()
        {
            ...
            {
                this.Menu = false;
                this.PageHeading = "Funder Report";
                this.ShowBranchName = Session[gblValue.BrnchCode].ToString() + " - " + Session[gblValue.BrName].ToString();
                this.ShowFinYear = Session[gblValue.FinYear].ToString() + " ( Login Date " + Session[gblValue.LoginDate].ToString() + " )";
                this.GetModuleByRole(mnuID.mnuFunderRpt);
        ...
        protected void btnExcl_Click(object sender, EventArgs e)
        {
           // GenerateRptData("Excel");
            //SetRptData("Excel");
            Int32 vRptFunder = Convert.ToInt32(Session[gblValue.RptFunder].ToString());
            if (vRptFunder != 0)
            {
                Int32 unixTicks = (Int32)(DateTime.UtcNow.Subtract(new DateTime(1970, 1, 1))).TotalSeconds;
                if (unixTicks - vRptFunder > 300)
                {
                    Session[gblValue.RptFunder] = ((Int32)(DateTime.UtcNow.Subtract(new DateTime(1970, 1, 1))).TotalSeconds).ToString();
                    GenerateRptData("Excel");
                }
                else
                {
        ...
        private void SetRptData(string pMode)
        {
            ...
            {
                oRpt = new CReports();
                dt = oRpt.rptFunder(vAsOnDt, vBrCode, Convert.ToInt32(Session[gblValue.UserId]));

                if (Convert.ToString(Session[gblValue.ViewAAdhar]) == "N")
                {
                    foreach (DataRow dr in dt.Rows) // search whole table
                    {
                        if (dr["Type Of Id Proof 1"].ToString() == "AADHAAR")
                        {
                            dr["ID Number 1"] = String.Format("{0}{1}", "********", Convert.ToString(dr["ID Number 1"]).Substring(Convert.ToString(dr["ID Number 1"]).Length - 4, 4));                     
                        }
                        if (dr["Type Of Id Proof 2"].ToString() == "AADHAAR")
                        {
                            dr["ID Number 2"] = String.Format("{0}{1}", "********", Convert.ToString(dr["ID Number 2"]).Substring(Convert.ToString(dr["ID Number 2"]).Length - 4, 4));                          
                        }
                    }
                }
                ...
                if (pMode == "Excel")
                {
                    string vFileNm = "attachment;filename=" + txtAsOnDt.Text + "_Funder.xls";
                    ...
                    foreach (DataRow dtrow in dt.Rows)
                    {
                        ...
                        {
                            if (dt.Columns[j].ColumnName == "Region Name" || dt.Columns[j].ColumnName == "ID Number 1" || dt.Columns[j].ColumnName == "ID Number 2")
                            {
        ...
        private void PrintTxt(DataTable dt)
        {
            ...
            string vFileNm = "";
            vFileNm = vFolderPath + "\\FunderReport_Centrum.txt";
        ...
        {
            string vBrCode = Convert.ToString(ViewState["BrCode"]);
            var req = new FunderReportRequest()
            {
                pAsOnDt = txtAsOnDt.Text,
                pBrCode = vBrCode,               
                pFormat = pMode,
                pViewAadhar=Convert.ToString(Session[gblValue.ViewAAdhar]),
            ...
            string vMsg = API.GenerateReport("GenerateFunder", Requestdata, vReportUrl);
    ...
    [DataContract]
    public class FunderReportRequest
    {
        [DataMember]
        ...
        [DataMember]
        public string pViewAadhar { get; set; }
        [DataMember]
