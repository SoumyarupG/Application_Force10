...
{
    public partial class rptPAR : CENTRUMBase
    {
        ...
        private void InitBasePage()
        {
            ...
            {
                this.Menu = false;
                this.PageHeading = "PAR Report";
                this.ShowBranchName = Session[gblValue.BrnchCode].ToString() + " - " + Session[gblValue.BrName].ToString();
                this.ShowFinYear = Session[gblValue.FinYear].ToString() + " ( Login Date " + Session[gblValue.LoginDate].ToString() + " )";
                this.GetModuleByRole(mnuID.mnuPARRpt);
        ...
        private void SetRptData(string pMode)
        {
            ...
            try
            {
                ...
                Int32 vRptPAR = Convert.ToInt32(Session[gblValue.RptPAR].ToString());
                if (vRptPAR != 0)
                {
                    Int32 unixTicks = (Int32)(DateTime.UtcNow.Subtract(new DateTime(1970, 1, 1))).TotalSeconds;
                    if (unixTicks - vRptPAR > 300)
                    {
                        Session[gblValue.RptPAR] = ((Int32)(DateTime.UtcNow.Subtract(new DateTime(1970, 1, 1))).TotalSeconds).ToString();
                        GenerateReport("GeneratePAR", Requestdata);
                    }
                    else
                    {
                ...
                //if (pMode == "Excel")
                //{
                //    string vFileNm = "attachment;filename="+txtAsOnDt.Text+"_PAR.xls";
                ...
                //        {
                //            if (dt.Columns[j].ColumnName == "Region Name" || dt.Columns[j].ColumnName == "LoanId")
                //            {
