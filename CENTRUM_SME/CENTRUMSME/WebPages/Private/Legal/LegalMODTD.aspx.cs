...
{
    public partial class LegalMODTD : CENTRUMBAse
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                txtMODTDVerRemark.Enabled = false;
        ...
        private void InitBasePage()
        {
            try
            {
                ...
                this.Menu = false;
                this.PageHeading = "LEGAL MODTD(Memorandum for Deposit of Title Deed)";
                ...
                {
                    Response.Redirect("~/WebPages/Public/PageAccess.aspx?mnuTxt=" + "LEGAL MODTD(Memorandum for Deposit of Title Deed)", false);
                }
            }
            catch
            {
        ...
        protected void gvPendMODTDList_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            String vSanId = "";
            DataTable dt = null;
            ...
            try
            {
                ...
                if (e.CommandName == "cmdShow")
                {
                    ...
                    dt = oCG.GetMODTDtlBySanctId(vSanId);
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        foreach (GridViewRow gr in gvPendMODTDList.Rows)
                        {
                            if ((gr.RowIndex) % 2 == 0)
                            {
                        ...
                        txtMODTDVerRemark.Enabled = true;
                       ...
                       // txtVerificDate.Enabled = false;
                        ...
                        if (dt.Rows[0]["IsMODTVerifyYN"].ToString() == "Y")
                            chkVerify.Checked = true;
                        else
                            chkVerify.Checked = false;
                        txtVerificDate.Text = dt.Rows[0]["MODTVerificationDate"].ToString();
                        if (dt.Rows[0]["IsMODTPrintYN"].ToString() == "Y")
                            chkPrint.Checked = true;
                        else
                            chkPrint.Checked = false;
                        txtPrintDate.Text = dt.Rows[0]["MODTPrintDate"].ToString();
                        txtMODTDVerRemark.Text = dt.Rows[0]["MODTRemark"].ToString();
        ...
        protected void gvMODTDone_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            String vSanId = "";
            DataTable dt = null;
            CApplication oCG = null;
                ...
                if (e.CommandName == "cmdShow")
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                       ...
                       // chkVerify.Enabled = true;
                        ...
                        txtMODTDVerRemark.Text = dt.Rows[0]["AgrVerRemarks"].ToString();
                        hdIsDisb.Value = dt.Rows[0]["IsDisbursed"].ToString();
                        tabLoanSanc.ActiveTabIndex = 3;
                    }
                }
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            string pSancId = "", pMODTVerifyYN = "", pMODTRemarks = "", pMODTPrintYN = "";
            ...
            if (chkPrint.Checked == true)
                ...
                pMODTPrintYN = "N";
            pMODTRemarks = txtMODTDVerRemark.Text.ToString();
            if (chkPrint.Checked == true && txtPrintDate.Text == "")
            {
                gblFuction.AjxMsgPopup("MODT Print Date can not be blank...");
                return;
            }
            if (chkVerify.Checked == true && txtVerificDate.Text == "")
            {
                gblFuction.AjxMsgPopup("MODT Verification Date can not be blank...");
                return;
            }
            DateTime? pMODTPrintDate = null;
            DateTime? pMODTVerificationDt = null;
            if (txtVerificDate.Text != "")
                pMODTVerificationDt = gblFuction.setDate(txtVerificDate.Text);
            if (txtPrintDate.Text != "")
                pMODTPrintDate = gblFuction.setDate(txtPrintDate.Text);
            CMember OMem = new CMember();
            Int32 vErr = 0;
            vErr = OMem.SaveMODTVerification(pSancId, pMODTVerifyYN, pMODTVerificationDt, pMODTPrintYN, pMODTPrintDate, pMODTRemarks, 0);
            if (vErr > 0)
            {
        ...
        private void PendMODTDBrCode()
        {
            DataTable dt = new DataTable();
            CDisburse oDisb = new CDisburse();
            dt = oDisb.GetBranchCodeForMODTD(Session[gblValue.BrnchCode].ToString(), "P");
            if (dt.Rows.Count > 0)
            {
                ddlPenMODTDLegBr.DataSource = dt;
                ddlPenMODTDLegBr.DataValueField = "BranchCode";
                ddlPenMODTDLegBr.DataTextField = "BranchName";
                ddlPenMODTDLegBr.DataBind();
                if (Session[gblValue.BrnchCode].ToString() == "0000")
                {
                    ListItem liSel = new ListItem("ALL", "0000");
                    ddlPenMODTDLegBr.Items.Insert(0, liSel);
                }
            }
            else
            {
                ddlPenMODTDLegBr.DataSource = null;
                ddlPenMODTDLegBr.DataBind();
            }
        }
        private void MODTDBrCode()
        {
            DataTable dt = new DataTable();
            CDisburse oDisb = new CDisburse();
            dt = oDisb.GetBranchCodeForMODTD(Session[gblValue.BrnchCode].ToString(), "A");
            if (dt.Rows.Count > 0)
            {
                ddlMODTDLegBr.DataSource = dt;
                ddlMODTDLegBr.DataValueField = "BranchCode";
                ddlMODTDLegBr.DataTextField = "BranchName";
                ddlMODTDLegBr.DataBind();
                if (Session[gblValue.BrnchCode].ToString() == "0000")
                {
                    ListItem liSel = new ListItem("ALL", "0000");
                    ddlMODTDLegBr.Items.Insert(0, liSel);
                }
            }
            else
            {
                ddlMODTDLegBr.DataSource = null;
                ddlMODTDLegBr.DataBind();
            }
        }
        ...
        private void PendingLegMODTList()
        {
            DataTable dt = null;
            ...
            try
            {
                ...
                dt = oCA.GetPenMODTList(ddlPenMODTDLegBr.SelectedValue.ToString());
                if (dt.Rows.Count > 0)
                {
                    gvPendMODTDList.DataSource = dt;
                    gvPendMODTDList.DataBind();
                }
                else
                {
                    gvPendMODTDList.DataSource = null;
                    gvPendMODTDList.DataBind();
                }
            }
            finally
        ...
        private void MODTList()
        {
            ...
            try
            {
                ...
                vBrCode = (string)Session[gblValue.BrnchCode];
                oCA = new CApplication();
                dt = oCA.GetMODTList(pFDate, pTDate, pSearch,ddlMODTDLegBr.SelectedValue.ToString());
                if (dt.Rows.Count > 0)
                {
                    gvMODTDone.DataSource = dt;
                    gvMODTDone.DataBind();
                }
                else
                {
                    gvMODTDone.DataSource = null;
                    gvMODTDone.DataBind();
                }
            }
            finally
        ...
        protected void ddlPenMODTDLegBr_SelectedIndexChanged(object sender, EventArgs e)
        {
            PendingLegMODTList();
        }
    }
