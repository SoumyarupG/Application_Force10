...
{
    public partial class RptCollectionEfficiency : CENTRUMBase
    {
        ...
        private void InitBasePage()
        {
            ...
            {
                this.Menu = false;
                this.PageHeading = "Collection Efficiency Report";
                this.ShowBranchName = Session[gblValue.BrnchCode].ToString() + " - " + Session[gblValue.BrName].ToString();
                this.ShowFinYear = Session[gblValue.FinYear].ToString() + " ( Login Date " + Session[gblValue.LoginDate].ToString() + " )";
                this.GetModuleByRole(mnuID.mnuCollEffiRpt);
                ...
                {
                    Response.Redirect("~/WebPages/Public/PageAccess.aspx?mnuTxt=" + "Collection Efficiency Report", false);
                }
            }
            catch
            {
        ...
        private void SetParameterForRptData(string pMode)
        {
            ...
            try
            {
                ...
                oRpt = new CReports();
                dt = oRpt.rptCollectionEfficiency(vBrCode, vFromDt, vToDt);
                if (dt.Rows.Count > 0)
                {
                    if (pMode == "Excel")
                    {
                        vFileNm = "attachment;filename=" + gblFuction.setDate(txtDtFrm.Text).ToString("yyyyMMdd") + "_ " + gblFuction.setDate(txtToDt.Text).ToString("yyyyMMdd") + "_Collection_Efficiency.xls";
                        ...
                        foreach (DataRow dtrow in dt.Rows)
                        {
                            ...
                            {
                                if (dt.Columns[j].ColumnName == "Bank Account No")
                                {
        ...
        protected void btnExcl_Click(object sender, EventArgs e)
        {
           // SetParameterForRptData("Excel");
            ...
            Int32 vRptCollectionEfficiency = Convert.ToInt32(Session[gblValue.RptCollectionEfficiency].ToString());
            if (vRptCollectionEfficiency != 0)
            {
                Int32 unixTicks = (Int32)(DateTime.UtcNow.Subtract(new DateTime(1970, 1, 1))).TotalSeconds;
                if (unixTicks - vRptCollectionEfficiency > 300)
                {
                    Session[gblValue.RptCollectionEfficiency] = ((Int32)(DateTime.UtcNow.Subtract(new DateTime(1970, 1, 1))).TotalSeconds).ToString();
                    GenerateRptData("Excel");
                }
                else
                {
        ...
        private void PrintTxt(DataTable dt)
        {
            ...
            string vFileNm = "";
            vFileNm = vFolderPath + "\\" + gblFuction.setDate(txtDtFrm.Text).ToString("yyyyMMdd") + "_ " + gblFuction.setDate(txtToDt.Text).ToString("yyyyMMdd") + "_Collection_Efficiency.txt";
        ...
        {
            string vBrCode = ViewState["BrCode"].ToString();
            var req = new RptCollectionEfficiencyCentrum()
            {
                pBranch = vBrCode,
                pDtFrm=txtDtFrm.Text,
                pToDt=txtToDt.Text,
                pFormat = pMode,
            ...
            string vMsg = API.GenerateReport("GenerateCollectionEfficiency", Requestdata, vReportUrl);
    ...
    [DataContract]
    public class RptCollectionEfficiencyCentrum
    {
        [DataMember]
        public string pBranch { get; set; }
        [DataMember]
        public string pDtFrm { get; set; }
        [DataMember]
