...
{
    public partial class RptInsuranceAtaGlance : CENTRUMBase
    {
        ...
        private void InitBasePage()
        {
            ...
            {
                this.Menu = false;
                this.PageHeading = "Insurance At a Glance";
                this.ShowBranchName = Session[gblValue.BrnchCode].ToString() + " - " + Session[gblValue.BrName].ToString();
                this.ShowFinYear = Session[gblValue.FinYear].ToString() + " ( Login Date " + Session[gblValue.LoginDate].ToString() + " )";
                this.GetModuleByRole(mnuID.mnuRptInsuAtaGlance);
        ...
        protected void btnExcel_Click(object sender, EventArgs e)
        {
            ...
            {
                oRpt = new CReports();              
                dt = oRpt.RptInsuranceAtaGlance(vBrCode, vAsOnDt);
                if (dt.Rows.Count > 0)
                {
                    vFileNm = "attachment;filename=" + gblFuction.setDate(txtAsDt.Text).ToString("yyyyMMdd") + "InsuranceAtaGlance.xls";
                    ...
                    Response.Write("<tr><td align=center' colspan='" + dt.Columns.Count + "'><b><font size='3'>Insurance At a Glance </font></b></td></tr>");
                    ...
                    foreach (DataRow dtrow in dt.Rows)
                    {
                        ...
                        {
                            if (dt.Columns[j].ColumnName == "MEMCODE" || dt.Columns[j].ColumnName == "LoanNo" || dt.Columns[j].ColumnName == "DISBURSEDATE" || dt.Columns[j].ColumnName == "DATEOFDEATH"
                                || dt.Columns[j].ColumnName == "Provisional Death Declare date" || dt.Columns[j].ColumnName == "ACCNO" || dt.Columns[j].ColumnName == "DOCSUBMISSIONDATE" || dt.Columns[j].ColumnName == "PAYMENTDATE")
                            {
