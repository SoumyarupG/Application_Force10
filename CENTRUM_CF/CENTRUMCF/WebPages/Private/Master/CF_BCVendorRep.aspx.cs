...
{
    public partial class CF_BCVendorRep : CENTRUMBAse
    {
        ...
        private void InitBasePage()
        {
            try
            {
                ...
                this.Menu = false;
                this.PageHeading = "BC Vendor Reports";
                this.ShowBranchName = Session[gblValue.BrnchCode].ToString() + " - " + Session[gblValue.BrName].ToString();
                this.ShowFinYear = Session[gblValue.FinYear].ToString() + " ( Login Date " + Session[gblValue.LoginDate].ToString() + " )";
                this.GetModuleByRole(mnuID.mnuCFBCvendorRep);
                ...
                {
                    Response.Redirect("~/WebPages/Public/PageAccess.aspx?mnuTxt=" + "BC Vendor Reports", false);
                }
            }
            catch
            {
        ...
        protected void gvEmpDoc_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                try
                {
                    ...
                    ddlDocType.SelectedIndex = ddlDocType.Items.IndexOf(ddlDocType.Items.FindByValue(e.Row.Cells[10].Text.Trim()));
                    ddlValue.SelectedIndex = ddlValue.Items.IndexOf(ddlValue.Items.FindByValue(e.Row.Cells[12].Text.Trim()));
        ...
        private Boolean GetDoc()
        {
            ...
            foreach (GridViewRow gr in gvEmpDoc.Rows)
            {
                ...
                DropDownList ddlValue = (DropDownList)gr.FindControl("ddlValue");
                if (gr.RowIndex <= gvEmpDoc.Rows.Count - 1)
                {
                    ...
                    {
                        gblFuction.MsgPopup("Please Select Comment");
                        return false;
                    }

                    if (txtDocRemarks.Text == "")
                    {
                        gblFuction.MsgPopup("Please Add Remarks");
                        return false;
                    }
                    ...
                    else
                    {
                        ...
                        {
                            dt.Rows[gr.RowIndex]["DocType"] = vDocType + vFUKYcDocType;//".png";
                        }
                        ...
                        dt.Rows[gr.RowIndex]["DocValue"] = Convert.ToString(ddlValue.SelectedValue);
                    }
        ...
        private void SetData()
        {
            ...
            foreach (DataRow gr in dt.Rows)
            {
                ...
                DropDownList ddlValue = (DropDownList)gvEmpDoc.Rows[i].FindControl("ddlValue");
                ...
                ddlValue.SelectedIndex = ddlValue.Items.IndexOf(ddlValue.Items.FindByValue(Convert.ToString(dt.Rows[i]["DocValue"])));
                i++;
            }
        }     
        private void ShowEmpRecords()
        {
            ...
            try
            {
                ...
                dt = oMem.CF_BCRepDtlByLeadId(Convert.ToInt64(vLoanAppId));
                if (dt.Rows.Count > 0)
                {
                    gvEmpDoc.DataSource = dt;
                    gvEmpDoc.DataBind();
        ...
        {
            Int64 vLeadID = Convert.ToInt64(hdnLoanAppNo.Value);
            string vXmlData;
            CMember oMem = null;
            int vErr = 0;
            ...
            if (GetDoc() == true)
            {
                ...
                vErr = oMem.CF_SaveBCRepDtl(vLeadID, gblFuction.setDate(Session[gblValue.LoginDate].ToString()), vXmlData, DocumentBucketURL, Mode, this.UserID);
                if (vErr == 0)
                {
                    if (Mode == "Save")
                        gblFuction.MsgPopup(gblPRATAM.SaveMsg);
        ...
        protected void GetVendorRep()
        {
            ...
            try
            {
                if (Session[gblValue.LeadID] != null)
                {
                    pLeadId = Convert.ToInt64(Session[gblValue.LeadID]);
                ...
                dt = oDist.CF_GetOtherRepDtlByLeadID(pLeadId);
                if (dt.Rows.Count > 0)
                {
                ...
                else
                {
                    ...
                    hdnLoanAppNo.Value = Convert.ToString(pLeadId);
                    txtLoanApplicationNo.Enabled = false;
                    if (Session[gblValue.BCPNO] != null)
                    {
                        txtLoanApplicationNo.Text = Convert.ToString(Session[gblValue.BCPNO]);
