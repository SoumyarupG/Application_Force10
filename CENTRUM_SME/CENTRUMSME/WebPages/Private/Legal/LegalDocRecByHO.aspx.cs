...
{
    public partial class LegalDocRecByHO : CENTRUMBAse
    {
        ...
        private void InitBasePage()
        {
            try
            {
                ...
                this.Menu = false;
                this.PageHeading = "LEGAL DOCUMENTS RECEIVE BY HEAD OFFICE";
                //this.ShowPageHeading = true;
                this.ShowBranchName = Session[gblValue.BrnchCode].ToString() + " - " + Session[gblValue.BrName].ToString();
                this.ShowFinYear = Session[gblValue.FinYear].ToString() + " ( Login Date " + Session[gblValue.LoginDate].ToString() + " )";
                //this.ShowHOMenu = false;
                this.GetModuleByRole(mnuID.mnuLegalDocRecByHO);
                ...
                {
                    Response.Redirect("~/WebPages/Public/PageAccess.aspx?mnuTxt=" + "LEGAL DOCUMENTS RECEIVE BY HEAD OFFICE", false);
                }
            }
            catch
            {
        ...
        private void PendDocRecBrCode()
        {
            DataTable dt = new DataTable();
            CDisburse oDisb = new CDisburse();
            dt = oDisb.GetBranchCodeForDocRec(Session[gblValue.BrnchCode].ToString(), "P");
            if (dt.Rows.Count > 0)
            {
                ddlPenLegDocRecBr.DataSource = dt;
                ddlPenLegDocRecBr.DataValueField = "BranchCode";
                ddlPenLegDocRecBr.DataTextField = "BranchName";
                ddlPenLegDocRecBr.DataBind();
                if (Session[gblValue.BrnchCode].ToString() == "0000")
                {
                    ListItem liSel = new ListItem("ALL", "0000");
                    ddlPenLegDocRecBr.Items.Insert(0, liSel);
                }
            }
            else
            {
                ddlPenLegDocRecBr.DataSource = null;
                ddlPenLegDocRecBr.DataBind();
            }
        }
        private void DocRecBrCode()
        {
            DataTable dt = new DataTable();
            CDisburse oDisb = new CDisburse();
            dt = oDisb.GetBranchCodeForDocRec(Session[gblValue.BrnchCode].ToString(), "A");
            if (dt.Rows.Count > 0)
            {
                ddlLegDocRecBr.DataSource = dt;
                ddlLegDocRecBr.DataValueField = "BranchCode";
                ddlLegDocRecBr.DataTextField = "BranchName";
                ddlLegDocRecBr.DataBind();
                if (Session[gblValue.BrnchCode].ToString() == "0000")
                {
                    ListItem liSel = new ListItem("ALL", "0000");
                    ddlLegDocRecBr.Items.Insert(0, liSel);
                }
            }
            else
            {
                ddlLegDocRecBr.DataSource = null;
                ddlLegDocRecBr.DataBind();
            }
        }
        ...
        protected void gvPendDocRecList_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            String vSanId = "";
            ...
            try
            {
                ...
                if (e.CommandName == "cmdShow")
                {
                    ...
                    ds = oCG.GetPenDocRecBySanctId(vSanId, Convert.ToInt32(Session[gblValue.UserId]));
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        foreach (GridViewRow gr in gvPendDocRecList.Rows)
                        {
                            if ((gr.RowIndex) % 2 == 0)
                            {
                        ...
                        txtDocSendBy.Text = dt.Rows[0]["SendBy"].ToString();
                        txtDocSendDate.Text = dt.Rows[0]["LegOrgDocSendDate"].ToString();
                        txtDocRecBy.Text = dt.Rows[0]["RecBy"].ToString();
        ...
        protected void gvDocRecList_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            String vSanId = "";
            DataSet ds = new DataSet();
            DataTable dt = new DataTable();
            ...
            try
            {
                ...
                {
                    oCG = new CApplication();
                    ds = oCG.GetDocRecBySanctId(vSanId);
                    if (ds.Tables.Count > 0)
                    {
                        dt = ds.Tables[0];
                        dt1 = ds.Tables[1];
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        txtCurierName.Text = dt.Rows[0]["CourierName"].ToString();
                        txtPODDocNo.Text = dt.Rows[0]["PODDocNo"].ToString();
                        txtDocRecDate.Text = dt.Rows[0]["RecDate"].ToString();
                        txtDocRecBy.Text = dt.Rows[0]["RecBy"].ToString();
                        hdIsDisb.Value = dt.Rows[0]["IsDisbursed"].ToString();
                        tabDocSend.ActiveTabIndex = 2;
                    }
        ...
        private Boolean SaveRecords(string Mode)
        {
            ...
            DateTime pDocRecDate = gblFuction.setDate(txtDocRecDate.Text);
            pCourierName = txtCurierName.Text.ToString();
            pDocNo = txtPODDocNo.Text.ToString();
            pDocRecBy = Convert.ToInt32(Session[gblValue.UserId]);
            CMember OMem = new CMember();
            Int32 vErr = 0;
            if (Mode == "Save")
            {
                vErr = OMem.SaveLegOrgDocSReceive(pSancId, pLoanAppNo, pCourierName, pDocNo, pDocRecDate, pDocRecBy, 0, "Save");
                if (vErr > 0)
                {
            ...
            {
                vErr = OMem.SaveLegOrgDocSReceive(pSancId, pLoanAppNo, pCourierName, pDocNo, pDocRecDate, pDocRecBy, 0, "Delete");
                if (vErr > 0)
                {
        ...
        private void PendingLegDocRecList()
        {
            DataTable dt = null;
            ...
            try
            {
                ...
                dt = oCA.GetPenOrgDocRecList(ddlPenLegDocRecBr.SelectedValue.ToString());
                if (dt.Rows.Count > 0)
                {
                    gvPendDocRecList.DataSource = dt;
                    gvPendDocRecList.DataBind();
                }
                else
                {
                    gvPendDocRecList.DataSource = null;
                    gvPendDocRecList.DataBind();
                }
            }
            finally
        ...
        private void LegDocRecList()
        {
            ...
            try
            {
                ...
                vBrCode = (string)Session[gblValue.BrnchCode];
                oCA = new CApplication();
                dt = oCA.GetOrgDocRecList(pFDate, pTDate, pSearch, ddlLegDocRecBr.SelectedValue.ToString());
                if (dt.Rows.Count > 0)
                {
                    gvDocRecList.DataSource = dt;
                    gvDocRecList.DataBind();
                }
                else
                {
                    gvDocRecList.DataSource = null;
                    gvDocRecList.DataBind();
                }
            }
            finally
        ...
        protected void ddlPenLegDocRecBr_SelectedIndexChanged(object sender, EventArgs e)
        {
            PendingLegDocRecList();
        }
