...
{
    public partial class RptDataEntry : CENTRUMBase
    {
        ...
        private void InitBasePage()
        {
            ...
            {
                this.Menu = false;
                this.PageHeading = "Data Entry Report";
                this.ShowBranchName = Session[gblValue.BrnchCode].ToString() + " - " + Session[gblValue.BrName].ToString();
                this.ShowFinYear = Session[gblValue.FinYear].ToString() + " ( Login Date " + Session[gblValue.LoginDate].ToString() + " )";
                this.GetModuleByRole(mnuID.mnuDataEntry);
                ...
                {
                    Response.Redirect("~/WebPages/Public/PageAccess.aspx?mnuTxt=" + "Data Entry Report", false);
                }
            }
            catch
            {
        ...
        private void SetParameterForRptData(string pMode)
        {
            ...
            {
                vDays = (vToDt - vFromDt).TotalDays;
                if (vDays == 0)
                {
                    gblFuction.AjxMsgPopup("Date diffrence should be minimum 1 days.");
                    return;
                }
                else if (vDays > 92)
                {
                    gblFuction.AjxMsgPopup("Date diffrence not more than 92 days.");
                    return;
                }
                else
                {
                    dt = oRpt.rptDataEntryDrillDown(vFromDt, vToDt, vBrCode);
                }
            }
            else
            {
                dt = oRpt.rptDataEntryMIS(vFromDt, vToDt, vBrCode);
            }

            if (pMode == "Excel")
            {
                string vFileNm = "attachment;filename=Data Entry Report.xls";
                ...
                Response.Write("<tr><td align=center' colspan='" + dt.Columns.Count + "'><b><font size='3'>Data Entry Report</font></b></td></tr>");
        ...
        private void PrintTxt(DataTable dt)
        {
            ...
            string vFileNm = "";
            vFileNm = vFolderPath + "\\DataEntry.txt";
