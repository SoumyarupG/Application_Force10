    ...
    public partial class VillageMaster : CENTRUMBAse
    {
        ...
        private void InitBasePage()
        {
            try
            {
                ...
                this.Menu = false;
                this.PageHeading = "City / Area Master";
                ...
                {
                    Response.Redirect("~/WebPages/Public/PageAccess.aspx?mnuTxt=" + "City / Area Master", false);
                }
            }
            catch
            {
        ...
        private void LoadGrid(Int32 pPgIndx)
        {
            ...
            try
            {
                ...
                oVlg = new CVillage();
                dt = oVlg.GetVillagePG(vBrCode, pPgIndx, ref vTotRows);
                gvVlg.DataSource = dt;
                gvVlg.DataBind();
        ...
        protected void gvVlg_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            try
            {
                vVlgId = Convert.ToString(e.CommandArgument);
                ...
                if (e.CommandName == "cmdShow")
                {
                    ...
                    oVlg = new CVillage();
                    dt = oVlg.GetVillageDetails(vVlgId);
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        txtVillage.Text = Convert.ToString(dt.Rows[0]["VillageName"]);
        ...
        private Boolean SaveRecords(string Mode)
        {
            ...
            string vVlgId = Convert.ToString(ViewState["VlgId"]);
            string vNewId ="";
            ...
            try
            {
                ...
                if (Mode == "Save")
                {
                    ...
                    vRec = oGbl.ChkDuplicate("VillageMst", "VillageName", txtVillage.Text.Replace("'", "''"), "DistrictId", vDistrictId.ToString(), "VillageId", vVlgId.ToString(), "Save");
                    if (vRec > 0)
                    {
                        gblFuction.MsgPopup("City/Area Can not be Duplicate...");
                        return false;
                    }
                    vErr = oVlg.SaveVillage(ref vNewId, vVlgId, txtVillage.Text.Replace("'", "''"), vDistrictId, vBrCode, this.UserID, "Save");
                    if (vErr > 0)
                    {
                ...
                else if (Mode == "Edit")
                {
                    oVlg = new CVillage();
                    oGbl = new CGblIdGenerator();
                    vRec = oGbl.ChkDuplicate("VillageMst", "VillageName", txtVillage.Text.Replace("'", "''"), "DistrictId", vDistrictId.ToString(), "VillageId", vVlgId.ToString(), "Edit");
                    if (vRec > 0)
                    {
                        gblFuction.MsgPopup("City/Area Can not be Duplicate...");
                        return false;
                    }
                    vErr = oVlg.SaveVillage(ref vNewId, vVlgId, txtVillage.Text.Replace("'", "''"), vDistrictId, vBrCode, this.UserID, "Edit");
                    if (vErr > 0)
                    {
                ...
                else if (Mode == "Delete")
                {
                    oVlg = new CVillage();
                    oGbl = new CGblIdGenerator();
                    vRec = oGbl.ChkDelete(vVlgId, "VillageID", "ClusterMst");
                    if (vRec > 0)
                    {
                        gblFuction.MsgPopup("The City/Area Has Cluster, You Can Not Delete The Village.");
                        return false;
                    }
                    vErr = oVlg.SaveVillage(ref vNewId, vVlgId, txtVillage.Text.Replace("'", "''"), vDistrictId, vBrCode, this.UserID, "Delet");
                    if (vErr > 0)
                    {
                        gblFuction.MsgPopup(gblPRATAM.DeleteMsg);
                        vResult = true;
