...
{
    public partial class BCNewMember : CENTRUMBase
    {
        ...
        private void InitBasePage()
        {
            ...
            {
                this.Menu = false;
                this.PageHeading = "BC New Member";
                this.ShowBranchName = Session[gblValue.BrnchCode].ToString() + " - " + Session[gblValue.BrName].ToString();
                this.ShowFinYear = Session[gblValue.FinYear].ToString() + " ( Login Date " + Session[gblValue.LoginDate].ToString() + " )";
                this.GetModuleByRole(mnuID.mnuBCNewMember);
                ...
                {
                    Response.Redirect("~/WebPages/Public/PageAccess.aspx?mnuTxt=" + "BC New Member", false);
                }
            }
            catch
            {
        ...
        private void SetRptData(string pMode)
        {
            ...
            try
            {
                ...
                dt = oRpt.rptBCNewMember(vAsOnDt, vBrCode);
                if (pMode == "Excel")
                {
                    string vFileNm = "attachment;filename=" + txtAsOnDt.Text + "_BCNewMember.xls";
                    ...
                    Response.Write("<tr><td align=center' colspan='" + dt.Columns.Count + "'><b><font size='3'> BC New Member </font></b></td></tr>");
                    ...
                    foreach (DataRow dtrow in dt.Rows)
                    {
                        ...
                        for (int j = 0; j < dt.Columns.Count; j++)
                        {
                            ...
                            {
                                if (dtrow["IDPROOFTYPE"].ToString() == "AADHAAR")
                                {
                                    dtrow["IDPROOFNUMBER"] = String.Format("{0}{1}", "********", Convert.ToString(dtrow["IDPROOFNUMBER"]).Substring(Convert.ToString(dtrow["IDPROOFNUMBER"]).Length - 4, 4)); //change the name                           
                                }

                                if (dtrow["ADDRESSPROOFTYPE"].ToString() == "AADHAAR")
                                {
                                    dtrow["ADDRESSPROOFNUMBER"] = String.Format("{0}{1}", "********", Convert.ToString(dtrow["ADDRESSPROOFNUMBER"]).Substring(Convert.ToString(dtrow["ADDRESSPROOFNUMBER"]).Length - 4, 4)); //change the name                           
                                }

                                if (dtrow["RELATEDCUSTOMERID1"].ToString().Length == 12)
                                {
                                    dtrow["RELATEDCUSTOMERID1"] = String.Format("{0}{1}", "********", Convert.ToString(dtrow["RELATEDCUSTOMERID1"]).Substring(Convert.ToString(dtrow["RELATEDCUSTOMERID1"]).Length - 4, 4));
                                }                                
                            }

                            if (dt.Columns[j].ColumnName == "IDPROOFNUMBER" || dt.Columns[j].ColumnName == "ADDRESSPROOFNUMBER" || dt.Columns[j].ColumnName == "RELATEDCUSTOMERID1")
                            {
        ...
        private void PrintTxt(DataTable dt)
        {
            ...
            string vFileNm = "";
            vFileNm = vFolderPath + "\\BCNewMember.txt";
