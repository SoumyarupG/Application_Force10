...
namespace CENTRUM.WebPages.Private.Transaction
{
    public partial class LoanRecovry : CENTRUMBase
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                if (Session["BrCntrl"] != null)
                {
                    ...
                    ViewState["AllowAdvYN"] = Convert.ToString(DtBrCntrl.Rows[0]["AdvCollJLG"]);
                }
                
                GetHOUser(Convert.ToInt32(hdUserID.Value));
                /*
        ...
        private void GetHOUser(Int32 pUserId)
        {
            DataTable dt = null;
            CLoanRecovery oLR = new CLoanRecovery();
            dt = oLR.GetHOUser(pUserId);
            if (dt.Rows.Count > 0)
            {
                hdHOUser.Value = Convert.ToString(dt.Rows[0]["HOUser"]);
            }
        }
        ...
        protected void btnShow_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (txtLoanNo.Text != "")
                {
                    ...
                    {
                        gblFuction.AjxMsgPopup("Invalid Loan No.");
                        return;
                    }
                }
        ...
        {
            DataTable dt = null, dtLoan = null, dtdesc = null, DtBrCntrl = null;
            CLoanRecovery oLR = null;
            ...
            try
            {
                ...
                foreach (GridViewRow gr in gvRecvry.Rows)
                {
                    ...
                    if (hdDescID.Value == "C0001" && vCollMode == "C" && Convert.ToString(DtBrCntrl.Rows[0]["CashCollJLG"]) == "N")
                    {
        ...
        protected void btnDel_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                oRole = new CRole();
                dtRole = oRole.GetRoleById(this.RoleId);
                if (dtRole.Rows.Count > 0)
                {
                    if (dtRole.Rows[0]["JlgDelColl"].ToString() == "N")
                    {
                        gblFuction.AjxMsgPopup("You do not have permission to delete the record.");
                        return;
                    }
                }
                //if (Session[gblValue.BrnchCode].ToString() != "0000")
        ...
        protected void btnReverse_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (dtRole.Rows.Count > 0)
                {
                    if (dtRole.Rows[0]["JlgRevColl"].ToString() == "N")
                    {
                        gblFuction.AjxMsgPopup("You do not have permission to reverse the record.");
                        return;
                    }
                }
                //if (Session[gblValue.BrnchCode].ToString() != "0000")
