...
using UnityVriddhiMobService.Service_Equifax;
using System.Xml;
using System.IO;
...
namespace UnityVriddhiMobService
{
    public class CRepository
    {
        ...
        #region SaveKarzaPanVerifyData
        public Int32 SaveKarzaPanVerifyData(string pPanId, string pResponseXml, string pBranchCode, string pEoid)
        {
            SqlCommand oCmd = new SqlCommand();
            Int32 vErr = 0;
            try
            {
                oCmd.CommandType = CommandType.StoredProcedure;
                oCmd.CommandText = "SaveKarzaPanVerifyData";
                DBUtility.AddParameter(oCmd.Parameters, ParameterDirection.Input, SqlDbType.VarChar, 30, "@pPanId", pPanId);
                DBUtility.AddParameter(oCmd.Parameters, ParameterDirection.Input, SqlDbType.VarChar, pResponseXml.Length + 1, "@pResponseXml", pResponseXml);
                DBUtility.AddParameter(oCmd.Parameters, ParameterDirection.Input, SqlDbType.VarChar, 4, "@pBranchCode", pBranchCode);
                DBUtility.AddParameter(oCmd.Parameters, ParameterDirection.Input, SqlDbType.VarChar, 12, "@pEoid", pEoid);
                DBUtility.AddParameter(oCmd.Parameters, ParameterDirection.Output, SqlDbType.Int, 4, "@pErr", 0);
        ...
        #region SaveKYC
        public string SaveKYC(PostKYCSaveData postKYCSaveData, PostVerifyData postverifyData, PostEMOCRData postEMOCRData)
        {
            SqlCommand oCmd1 = new SqlCommand();
            SqlCommand oCmd2 = new SqlCommand();
            Int32 vErr1 = 0, pCbId = 0, pCoAppCBId = 0, vErr = 0;
            string pErrDesc = "", pEnqId = "XX", vMemDob = string.Empty, pCBErrDesc = "";
            ...
            string vHighmarkJsonResponse = "", vHighmarkXml = "", pStatusDesc = "", vMsg = "";
            ...
            if (Convert.ToInt32(postKYCSaveData.pId1Type) == 1 || Convert.ToInt32(postKYCSaveData.pId2Type) == 1)
            {
                ...
                vApplAadhaarRefNo = AadhaarVaultCalling(vApplAadharNo, postKYCSaveData.pMobileNo1, postKYCSaveData.pCreatedBy);
            ...
            if (Convert.ToInt32(postKYCSaveData.pCoAppIdentyPRofId) == 1 || Convert.ToInt32(postKYCSaveData.pCoAppAddressPRofId) == 1)
            {
                vCoApplAadharNo = Convert.ToInt32(postKYCSaveData.pCoAppIdentyPRofId) == 1 ? postKYCSaveData.pCoAppIdentyProfNo : postKYCSaveData.pCoAppAddressProfNo;
                vCoApplMaskedAadhaar = String.Format("{0}{1}", "********", vCoApplAadharNo.Substring(vCoApplAadharNo.Length - 4, 4));
                vCoApplAadhaarRefNo = AadhaarVaultCalling(vCoApplAadharNo, postKYCSaveData.pCoAppMobileNo, postKYCSaveData.pCreatedBy);
                if (vCoApplAadhaarRefNo != "")
                {
                    //postKYCSaveData.pCoAppIdentyPRofId = "13";
                    //postKYCSaveData.pCoAppIdentyProfNo = vCoApplAadhaarRefNo;

                    postKYCSaveData.pCoAppIdentyProfNo = Convert.ToInt32(postKYCSaveData.pCoAppIdentyPRofId) == 1 ? vApplAadhaarRefNo : postKYCSaveData.pCoAppIdentyProfNo;
                    postKYCSaveData.pCoAppAddressProfNo = Convert.ToInt32(postKYCSaveData.pCoAppAddressPRofId) == 1 ? vApplAadhaarRefNo : postKYCSaveData.pCoAppAddressProfNo;
                    postKYCSaveData.pCoAppIdentyPRofId = Convert.ToInt32(postKYCSaveData.pCoAppIdentyPRofId) == 1 ? "13" : postKYCSaveData.pCoAppIdentyPRofId;
                    postKYCSaveData.pCoAppAddressPRofId = Convert.ToInt32(postKYCSaveData.pCoAppAddressPRofId) == 1 ? "13" : postKYCSaveData.pCoAppAddressPRofId;
                }
                else
                {
                    return "Failed:Unable to get aadhaar ref no." + ":" + pEnqId + ":" + pCbId;
            ...
            try
            {
                ...
                DBUtility.AddParameter(oCmd1.Parameters, ParameterDirection.Input, SqlDbType.Int, 4, "@pCoAppAddressPRofId", Convert.ToInt32(postKYCSaveData.pCoAppAddressPRofId));
                DBUtility.AddParameter(oCmd1.Parameters, ParameterDirection.Input, SqlDbType.VarChar, 50, "@pCoAppAddressProfNo", postKYCSaveData.pCoAppAddressProfNo);
                ...
                if (vErr1 == 0)
                {
                    if ((postKYCSaveData.pOCRApproveYN).Trim() == "Y" && (postKYCSaveData.pKarzaVerifyYN).Trim() == "Y")
                    {
                        VerifyHighmark(pEnqId, "C", postKYCSaveData.pBranchCode, postKYCSaveData.pDate, postKYCSaveData.pCreatedBy);
                        VerifyHighmark(pEnqId, "E", postKYCSaveData.pBranchCode, postKYCSaveData.pDate, postKYCSaveData.pCreatedBy);
                        VerifyHighmark(pEnqId, "A", postKYCSaveData.pBranchCode, postKYCSaveData.pDate, postKYCSaveData.pCreatedBy);
                    ...
                    postverifyData.EnquiryId = pEnqId;
                    //postEMOCRData.EnquiryId = pEnqId;
                    try
                    {
                        SaveVerifyData(postverifyData);
            ...
            string pContactNo, string pAddress, string pDistrictName, string pStateCode, string pPinCode)
        {
            ...
            string userId = "unity_vyapar";//UAT
            string password = "2045387DE0A6EE24C15371FEF3DB7642F644E074";//UAT
            string mbrid = "NBF0003171";
            string productType = "INDV";
            string productVersion = "2.0";
            string reqVolType = "INDV";
            string SUB_MBR_ID = "UNITY SMALL FINANCE BANK";//UAT
            string request_datetime = DateTime.Now.ToString("dd-MM-yyyy HH:mm:ss");
            string vAppDate = pAsOnDate;

            string HEADER_SEGMENT = "<HEADER-SEGMENT>"
                                   + "<PRODUCT-TYP>" + productType + "</PRODUCT-TYP>"
                                   + "<PRODUCT-VER>" + productVersion + "</PRODUCT-VER>"
                                   + "<REQ-MBR></REQ-MBR>"
                                   ...
                                   + "<LOS-NAME>DIRECT</LOS-NAME>"
                                   + "<LOS-VENDER>SALESFORCE</LOS-VENDER>"
                                   + "<MFI>"
                                   + "<INDV>TRUE</INDV>"
                                   + "<SCORE>FALSE</SCORE>"
                                   + "<GROUP>FALSE</GROUP>"
            ...
            string APPLICANT_SEGMENT = "<APPLICANT-SEGMENT>"
                                        + "<APPLICANT-NAME><NAME1>" + pMemberName + "</NAME1>"
                                        + "<NAME2></NAME2><NAME3></NAME3><NAME4></NAME4><NAME5></NAME5></APPLICANT-NAME>"
                                        + "<DOB><DOB-DATE>" + pDOB + "</DOB-DATE>"
                                        + "<AGE>" + pAge + "</AGE>"
                                        + "<AGE-AS-ON>" + pAsOnDate + "</AGE-AS-ON></DOB>"
                                        + "<IDS>"
                                        + ((pAadhaar == "") ? "" : "<ID><TYPE>ID03</TYPE><VALUE>" + pAadhaar + "</VALUE></ID>") //aadhaar no
                                        + ((pPanId == "") ? "" : "<ID><TYPE>ID07</TYPE><VALUE>" + pPanId + "</VALUE></ID>") //pan no (if available)
                                        + ((pVoterId == "") ? "" : "<ID><TYPE>ID02</TYPE><VALUE>" + pVoterId + "</VALUE></ID>") //voter id (if available)
                                        + ((pRationCard == "") ? "" : "<ID><TYPE>ID05</TYPE><VALUE>" + pRationCard + "</VALUE></ID>") //ration card (if available)
                                        + "</IDS>"
                                        + "<RELATIONS><RELATION><NAME>" + pRelativeName + "</NAME><TYPE>" + pRelationCode + "</TYPE>"
                                        + "</RELATION></RELATIONS><PHONES><PHONE><TELE-NO>" + pContactNo + "</TELE-NO>"
                                        + "<TELE-NO-TYPE>MOBILE PHONE</TELE-NO-TYPE></PHONE></PHONES>"
                                        + "<EMAILS>"
                                        + "<EMAIL></EMAIL>"
                                        + "</EMAILS>"
                                        + "<GENDER>G02</GENDER>"
            ...
            string ADDRESS_SEGMENT = "<ADDRESS-SEGMENT><ADDRESS><TYPE>D01</TYPE><ADDRESS-1>" + pAddress + "</ADDRESS-1>"
            + "<CITY>" + pDistrictName + "</CITY><STATE>" + pStateCode + "</STATE><PIN>" + pPinCode + "</PIN></ADDRESS></ADDRESS-SEGMENT>";
            ...
            string APPLICATION_SEGMENT = "<APPLICATION-SEGMENT><INQUIRY-UNIQUE-REF-NO>" + pEnqId + "/" + DateTime.Now.ToString("ddMMyyyyHHmmss") + "</INQUIRY-UNIQUE-REF-NO>"
                                       ...
                                       + "<CREDT-RPT-TRN-DT-TM>" + DateTime.Now.ToString("ddMMyyyyHHmmss") + "</CREDT-RPT-TRN-DT-TM>"
                                       + "<CREDT-INQ-PURPS-TYP-DESC>A37</CREDT-INQ-PURPS-TYP-DESC>"
            ...
            try
            {
                ...
                //----------------------------------------------------------
                var httpWebRequest = (HttpWebRequest)WebRequest.Create("https://test.crifhighmark.com/Inquiry/doGet.service/requestResponseSync");//TEST
                //var httpWebRequest = (HttpWebRequest)WebRequest.Create("https://hub.crifhighmark.com/Inquiry/doGet.service/requestResponseSync");//LIVE
        ...
        #region VerifyHighmark
        public string VerifyHighmark(string pEnqId, string pCustType, string pBranchCode, string pLogDt, string pCreatedBy)
        {
            DataTable dt = new DataTable();
            string vMsg = "", pStatusDesc = "";
            string pEqXml = "";
            string vHighmarkJsonResponse = "";

            int vErr = 0;
            try
            {
                //pMode = "A"-Customer/Applicant,"C"-CoApplicant,"E"- Earning Member
                dt = GetMemberInfo(pEnqId, "H", pCustType);
                if (dt.Rows.Count > 0)
                {
                    try
                    {
                        ////*************************** For Live ***************************************************      
                        vHighmarkJsonResponse = HighMarkProcess(pEnqId, dt.Rows[0]["MemberName"].ToString(), dt.Rows[0]["DOB"].ToString(), dt.Rows[0]["Age"].ToString()
                            , dt.Rows[0]["AsOnDate"].ToString(), dt.Rows[0]["Aadhaar"].ToString(), dt.Rows[0]["Pan"].ToString(), dt.Rows[0]["Voter"].ToString(), ""
                            , dt.Rows[0]["RelativeName"].ToString(), dt.Rows[0]["RelationCode"].ToString(), dt.Rows[0]["ContactNo"].ToString()
                            , dt.Rows[0]["Address"].ToString(), dt.Rows[0]["DistrictName"].ToString(), dt.Rows[0]["StateCode"].ToString(), dt.Rows[0]["PinCode"].ToString());
                        ...
                        if (vErr == 1)
                        {
                            return "Highmark Verification Successful" + ";" + pStatusDesc;
                        }
                        else if (vErr == 5)
                        {
                            return "Highmark Verification Failed" + ";" + pStatusDesc;
                        }
                        else
                        {
                            return "Highmark Verification Failed" + ";" + pStatusDesc;
                    ...
                    {
                        vErr = UpdateEquifaxInformation(pEnqId, Convert.ToInt32(dt.Rows[0]["CBID"]), pEqXml, pBranchCode, "", Convert.ToInt32(pCreatedBy), pLogDt, ref pStatusDesc);
                        return "Highmark Verification Failed" + ":" + ex.ToString();
                    }
                }
                else
                {
        ...
        public string VerifyEquifax(string pEnqId, string pCustType, string pBranchCode, string pLogDt, string pCreatedBy, string pAadhaarNo)
        {
            DataTable dt = new DataTable();
            string vMsg = "", pStatusDesc = "";
        ...
        #region VerifyData
        public string SaveVerifyData(PostVerifyData vVerifyData)
        {
            string vApplID1Response, xmlvApplID1Response, vApplID2Response, xmlvApplID2Response, vCoApplID1Response, xmlCovApplID1Response, vCoApplID2Response, xmlCovApplID2Response;

            SqlCommand oCmd = new SqlCommand();
            Int32 vErr = 0;
            try
            {
                vApplID1Response = vVerifyData.ApplID1Response == null ? "" : vVerifyData.ApplID1Response.Replace("\u0000", "");
                vApplID1Response = vApplID1Response.Replace("\\u0000", "");
                xmlvApplID1Response = AsString(JsonConvert.DeserializeXmlNode(vApplID1Response, "root"));

                vApplID2Response = vVerifyData.ApplID2Response == null ? "" : vVerifyData.ApplID2Response.Replace("\u0000", "");
                vApplID2Response = vApplID2Response.Replace("\\u0000", "");
                xmlvApplID2Response = AsString(JsonConvert.DeserializeXmlNode(vApplID2Response, "root"));

                vCoApplID1Response = vVerifyData.CoApplID1Response == null ? "" : vVerifyData.CoApplID1Response.Replace("\u0000", "");
                vCoApplID1Response = vCoApplID1Response.Replace("\\u0000", "");
                xmlCovApplID1Response = AsString(JsonConvert.DeserializeXmlNode(vCoApplID1Response, "root"));

                vCoApplID2Response = vVerifyData.CoApplID2Response == null ? "" : vVerifyData.CoApplID2Response.Replace("\u0000", "");
                vCoApplID2Response = vCoApplID2Response.Replace("\\u0000", "");
                xmlCovApplID2Response = AsString(JsonConvert.DeserializeXmlNode(vCoApplID2Response, "root"));

                oCmd.CommandType = CommandType.StoredProcedure;
                oCmd.CommandText = "SaveVerifyData";

                DBUtility.AddParameter(oCmd.Parameters, ParameterDirection.Input, SqlDbType.VarChar, 12, "@pEnquiryId", vVerifyData.EnquiryId == null ? "" : vVerifyData.EnquiryId);

                DBUtility.AddParameter(oCmd.Parameters, ParameterDirection.Input, SqlDbType.Int, 4, "@pApplID1Id", Convert.ToInt32(vVerifyData.ApplID1Id));
                if (xmlvApplID1Response == null)
                    DBUtility.AddParameter(oCmd.Parameters, ParameterDirection.Input, SqlDbType.Xml, 6, "@pApplID1Response", DBNull.Value);
                else
                    DBUtility.AddParameter(oCmd.Parameters, ParameterDirection.Input, SqlDbType.Xml, xmlvApplID1Response.Length + 1, "@pApplID1Response", xmlvApplID1Response);

                DBUtility.AddParameter(oCmd.Parameters, ParameterDirection.Input, SqlDbType.Int, 4, "@pApplID2Id", Convert.ToInt32(vVerifyData.ApplID2Id));
                if (xmlvApplID2Response == null)
                    DBUtility.AddParameter(oCmd.Parameters, ParameterDirection.Input, SqlDbType.Xml, 6, "@pApplID2Response", DBNull.Value);
                else
                    DBUtility.AddParameter(oCmd.Parameters, ParameterDirection.Input, SqlDbType.Xml, xmlvApplID2Response.Length + 1, "@pApplID2Response", xmlvApplID2Response);

                DBUtility.AddParameter(oCmd.Parameters, ParameterDirection.Input, SqlDbType.Int, 4, "@pCoApplID1Id", Convert.ToInt32(vVerifyData.CoApplID1Id));
                if (xmlCovApplID1Response == null)
                    DBUtility.AddParameter(oCmd.Parameters, ParameterDirection.Input, SqlDbType.Xml, 6, "@pCoApplID1Response", DBNull.Value);
                else
                    DBUtility.AddParameter(oCmd.Parameters, ParameterDirection.Input, SqlDbType.Xml, xmlCovApplID1Response.Length + 1, "@pCoApplID1Response", xmlCovApplID1Response);

                DBUtility.AddParameter(oCmd.Parameters, ParameterDirection.Input, SqlDbType.Int, 4, "@pCoApplID2Id", Convert.ToInt32(vVerifyData.CoApplID2Id));
                if (xmlCovApplID2Response == null)
                    DBUtility.AddParameter(oCmd.Parameters, ParameterDirection.Input, SqlDbType.Xml, 6, "@pCoApplID2Response", DBNull.Value);
                else
                    DBUtility.AddParameter(oCmd.Parameters, ParameterDirection.Input, SqlDbType.Xml, xmlCovApplID2Response.Length + 1, "@pCoApplID2Response", xmlCovApplID2Response);
        ...
        public string SavePdBySO(PostPdBySo postPdBySO)
        {
            ...
            try
            {
                ...
                DBUtility.AddParameter(oCmd.Parameters, ParameterDirection.Input, SqlDbType.VarChar, 50, "@pUdyamAadhaarRegistNo", postPdBySO.pUdyamAadhaarRegNo);
        ...
        {
            string postURL = vDBName.ToUpper() == "CENTRUM_SARAL" ? "https://aml.unitybank.co.in/ramp/webservices/createToken" : "https://jocatauat.unitybank.co.in/ramp/webservices/createToken";
            try
            {
        ...
        public string RampRequest(PostRampRequest postRampRequest)
        {
            ...
            try
            {
                ...
                //-----------------------Ramp Request------------------------
                string postURL = vDBName.ToUpper() == "CENTRUM_SARAL" ? "https://aml.unitybank.co.in/ramp/webservices/request/handle-ramp-request" : "https://jocatauat.unitybank.co.in/ramp/webservices/request/handle-ramp-request";
        ...
        {
            string postURL = vDBName.ToUpper() == "CENTRUM_SARAL" ? "https://avault.unitybank.co.in/vault/insert" : "https://avaultuat.unitybank.co.in/vault/insert";
        ...
        public AadhaarNoByRefId GetAadhaarNoByRefId(AadhaarNoReq pAadhaarNoReq)
        {
            ...
            try
            {
                ...
                string postURL = vDBName.ToUpper() == "CENTRUM_SARAL" ? "https://avault.unitybank.co.in/vault/get-by-refid?" + vPostData : "https://avaultuat.unitybank.co.in/vault/get-by-refid?" + vPostData;
                var vStrMycertPub = System.Web.Hosting.HostingEnvironment.MapPath("~/Certificate/adharvault_unity.pfx");
                string password = "3652145879";
                ServicePointManager.SecurityProtocol = (SecurityProtocolType)(0xc0 | 0x300 | 0xc00);
        ...
        public ProsidexResponse ProsidexSearchCustomer(ProsidexRequest prosidexRequest)
        {
            ...
            try
            {
                ...
                string postURL = vDBName.ToUpper() == "CENTRUM_SARAL" ? "https://ucic.unitybank.co.in:9002/UnitySfbWS/searchCustomer" : "http://144.24.116.182:9002/UnitySfbWS/searchCustomer";
                ServicePointManager.SecurityProtocol = (SecurityProtocolType)(0xc0 | 0x300 | 0xc00);
                HttpWebRequest request = WebRequest.Create(postURL) as HttpWebRequest;
                request.Method = "POST";
                request.ContentType = "application/json";
        ...
        public string JocataRiskCat(RiskCatReq vRiskCatReq)
        {
            ...
            {
                //------------------------------------URI----------------------------------------------
                string postURL = vDBName.ToUpper() == "CENTRUM_SARAL" ? "https://aml.unitybank.co.in/orck/on-boarding/calculate-risk"
                   : "https://jocatauat.unitybank.co.in/orck/on-boarding/calculate-risk";
                string vToken = vDBName.ToUpper() == "CENTRUM_SARAL" ? "611d9587-7546-8e62-1809-c8f8c193d421" : "596cf388-a04f-53b1-ce6b-e9a54082363f";
                //------------------------------------------------------------------------------------
