...
{
    public partial class BCSanctionMIS : CENTRUMBase
    {
        ...
        private void InitBasePage()
        {
            ...
            {
                this.Menu = false;
                this.PageHeading = "BC Sanction MIS";
                this.ShowBranchName = Session[gblValue.BrnchCode].ToString() + " - " + Session[gblValue.BrName].ToString();
                this.ShowFinYear = Session[gblValue.FinYear].ToString() + " ( Login Date " + Session[gblValue.LoginDate].ToString() + " )";
                this.GetModuleByRole(mnuID.mnuBCSanctionMIS);
                ...
                {
                    Response.Redirect("~/WebPages/Public/PageAccess.aspx?mnuTxt=" + "BC Sanction MIS", false);
                }
            }
            catch
            {
        ...
        private void SetRptData(string pMode)
        {
            ...
            try
            {
                ...
                dt = oRpt.rptBCSanction(vAsOnDt, vBrCode);
                if (pMode == "Excel")
                {
                    string vFileNm = "attachment;filename=" + txtAsOnDt.Text + "_BCSanctionMIS.xls";
                    ...
                    Response.Write("<tr><td align=center' colspan='" + dt.Columns.Count + "'><b><font size='3'> BC Sanction MIS Report </font></b></td></tr>");
                    ...
                    foreach (DataRow dtrow in dt.Rows)
                    {
                        ...
                        for (int j = 0; j < dt.Columns.Count; j++)
                        {
                            ...
                            {
                                if (dtrow["ID 1 Type"].ToString() == "AADHAAR")
                                {
                                    dtrow["ID 1 Number"] = String.Format("{0}{1}", "********", Convert.ToString(dtrow["ID 1 Number"]).Substring(Convert.ToString(dtrow["ID 1 Number"]).Length - 4, 4));            
                                }

                                if (dtrow["ID 2 type"].ToString() == "AADHAAR")
                                {
                                    dtrow["ID 2 number"] = String.Format("{0}{1}", "********", Convert.ToString(dtrow["ID 2 number"]).Substring(Convert.ToString(dtrow["ID 2 number"]).Length - 4, 4));          
                                }

                                if (dtrow["ID 3 type"].ToString() == "AADHAAR")
                                {
                                    dtrow["ID 3 number"] = String.Format("{0}{1}", "********", Convert.ToString(dtrow["ID 3 number"]).Substring(Convert.ToString(dtrow["ID 3 number"]).Length - 4, 4));                
                                }
                            }
                            if (dt.Columns[j].ColumnName == "ID 1 Number" || dt.Columns[j].ColumnName == "ID 2 number" || dt.Columns[j].ColumnName == "ID 3 number")
                            {
        ...
        private void PrintTxt(DataTable dt)
        {
            ...
            string vFileNm = "";
            vFileNm = vFolderPath + "\\BCSanctionMIS.txt";
