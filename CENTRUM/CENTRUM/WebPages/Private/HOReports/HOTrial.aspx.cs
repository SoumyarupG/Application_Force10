...
{
    public partial class HOTrial : CENTRUMBase
    {
        ...
        private void InitBasePage()
        {
            try
            {
                ...
                this.GetModuleByRole(mnuID.mnuHOTrialBal);
                ...
                {
                    Response.Redirect("~/WebPages/Public/PageAccess.aspx?mnuTxt=" + "Consolidate Trial Balance", false);
                }
            }
            catch
            {
        ...
        private void GetData(string pFormat)
        {
            ...
            string vRptPath = "";
            if (rbBranchWise.SelectedValue == "1")
                vRptPath = Request.PhysicalApplicationPath.ToString() + "Reports\\Trial.rpt";
            else
                vRptPath = Request.PhysicalApplicationPath.ToString() + "Reports\\TrialBranchWise.rpt";
            ...
            if (rbBranchWise.SelectedValue == "1")
                dt = oRpt.rptBranchWiseTrial(vAcMst, vAcDtl, gblFuction.setStrDate(Session[gblValue.FinFromDt].ToString()), gblFuction.setStrDate(txtFrmDt.Text), gblFuction.setStrDate(txtToDt.Text), vBrCode, vFinYr);
            else
                dt = oRpt.rptTrial(vAcMst, vAcDtl, gblFuction.setStrDate(Session[gblValue.FinFromDt].ToString()), gblFuction.setStrDate(txtFrmDt.Text), gblFuction.setStrDate(txtToDt.Text), vBrCode, vFinYr);
            if (pFormat == "PDF")
            {
                using (ReportDocument rptDoc = new ReportDocument())
                {
                    ...
                    rptDoc.SetParameterValue("pTitle", "Consolidate Trial Balance");
                    ...
                    //if (pFormat == "PDF")
                    rptDoc.ExportToHttpResponse(ExportFormatType.PortableDocFormat, Response, true, gblFuction.setDate(txtToDt.Text).ToString("yyyyMMdd") + "_Consolidate_Trial_Balance");
                    //else
                    //    rptDoc.ExportToHttpResponse(ExportFormatType.Excel, Response, true, gblFuction.setDate(txtToDt.Text).ToString("yyyyMMdd") + "_Consolidate_Trial_Balance");
            ...
            else if (pFormat == "Excel")
            {
                ...
                Response.Write("<tr><td align=center' colspan='" + dt.Columns.Count + "'><b><font size='3'> Consolidate Trial Balance </font></b></td></tr>");
        ...
        protected void btnExcl_Click(object sender, EventArgs e)
        {
            ...
            {
                //GetData("Excel");
                Int32 vHOTrial = Convert.ToInt32(Session[gblValue.HOTrial].ToString());
                if (vHOTrial != 0)
                {
                    Int32 unixTicks = (Int32)(DateTime.UtcNow.Subtract(new DateTime(1970, 1, 1))).TotalSeconds;
                    if (unixTicks - vHOTrial > 300)
                    {
                        Session[gblValue.HOTrial] = ((Int32)(DateTime.UtcNow.Subtract(new DateTime(1970, 1, 1))).TotalSeconds).ToString();
                        GenerateRptData("Excel","N");
                    }
                    else
                    {
        ...
        private void GenerateRptData(string pMode, string ptype)
        {
            ...
            string vAcMst = Session[gblValue.ACVouMst].ToString();
            string vAcDtl = Session[gblValue.ACVouDtl].ToString();
            string vRptType = "T", vBranchWise="N";
            if (rbDtlsSumm.SelectedValue == "1")
                vRptType = "S";
    ...
    [DataContract]
    public class HoTrialCentrum
    {
        [DataMember]
        public string TableMst { get; set; }
        [DataMember]
        public string TableDtl { get; set; }
        [DataMember]
        ...
        [DataMember]
        public string pBranchWise { get; set; }
