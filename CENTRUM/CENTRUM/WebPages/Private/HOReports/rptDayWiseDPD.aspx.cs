...
{
    public partial class rptDayWiseDPD : CENTRUMBase
    {
        ...
        private void InitBasePage()
        {
            ...
            {
                this.Menu = false;
                this.PageHeading = "Day Wise DPD";
                this.ShowBranchName = Session[gblValue.BrnchCode].ToString() + " - " + Session[gblValue.BrName].ToString();
                this.ShowFinYear = Session[gblValue.FinYear].ToString() + " ( Login Date " + Session[gblValue.LoginDate].ToString() + " )";
                this.GetModuleByRole(mnuID.mnuDayWiseDPD);
                ...
                {
                    Response.Redirect("~/WebPages/Public/PageAccess.aspx?mnuTxt=" + "Day Wise DPD", false);
                }
            }
            catch
            {
        ...
        {
            //SetRptData("Excel");
            Int32 vRptDayWiseDPD = Convert.ToInt32(Session[gblValue.RptDayWiseDPD].ToString());
            if (vRptDayWiseDPD != 0)
            {
                Int32 unixTicks = (Int32)(DateTime.UtcNow.Subtract(new DateTime(1970, 1, 1))).TotalSeconds;
                if (unixTicks - vRptDayWiseDPD > 300)
                {
                    Session[gblValue.RptDayWiseDPD] = ((Int32)(DateTime.UtcNow.Subtract(new DateTime(1970, 1, 1))).TotalSeconds).ToString();
                    SetRptData("Excel");
                }
                else
                {
        ...
        private void SetRptData(string pMode)
        {
            ...
            string vBrCode = Convert.ToString(ViewState["BrCode"]), vProjectType = "";
            try
            {
                ...
                if (rblProjectType.SelectedValue == "rbJLG")
                    vProjectType = "JLG";
                else
                    vProjectType = "MEL";
                oRpt = new CReports();

                //dt = oRpt.rptPar(vAsOnDt, vBrCode);

                var req = new DayWiseDPDReportRequest()
                {

                    pFrmDt = txtDtFrm.Text,
                    pToDt = txtToDt.Text,
                    pBrCode = vBrCode,
                    pFormat = pMode,
                    pProjectType = vProjectType,
                ...
                GenerateReport("GenerateDayWiseDPD", Requestdata);
        ...
        private void PrintTxt(DataTable dt)
        {
            ...
            string vFileNm = "";
            vFileNm = vFolderPath + "\\PARReport_Centrum.txt";
    ...
    public class DayWiseDPDReportRequest
    {
        public string pFrmDt { get; set; }
