...
{
    public partial class OccupationMaster : CENTRUMBAse
    {
        ...
        private void InitBasePage()
        {
            try
            {
                ...
                this.Menu = false;
                this.PageHeading = "Occupation Master";
        ...
        {
            Int32 vOccuId = 0, vRow = 0;
            DataTable dt = null;
            COccu oOcu = null;
            try
            {
                vOccuId = Convert.ToInt32(e.CommandArgument);
                ViewState["OccuId"] = vOccuId;
                if (e.CommandName == "cmdShow")
                {
                    ...
                    **/
                    ...
                    oOcu = new COccu();
                    dt = oOcu.GetOccubyId(vOccuId);
                    if (dt.Rows.Count > 0)
                    {
                        txtOccu.Text = Convert.ToString(dt.Rows[vRow]["Occupation"]).Trim();
        ...
        private Boolean SaveRecords(string Mode)
        {
            ...
            string vOccuId = Convert.ToString(ViewState["OccuId"]);
            Int32 vErr = 0, vRec = 0,vOcuId = 0;
            COccu oOcu = null;
            CGblIdGenerator oGbl = null;
            try
            {
                vOcuId = Convert.ToInt32(ViewState["OccuId"]);
                if (Mode == "Save")
                {
                    oOcu = new COccu();
                    oGbl = new CGblIdGenerator();
                    vRec = oGbl.ChkDuplicate("OccupationMst", "Occupation", txtOccu.Text.Replace("'", "''"), "", "", "OccupationId", vOccuId, "Save");
                    if (vRec > 0)
                    {
                        gblFuction.MsgPopup("Occupation Can not be Duplicate...");
                        return false;
                    }
                    vErr = oOcu.InsertOccu(ref vOcuId,txtOccu.Text.Replace("'","''"),this.UserID,"I","Save");
                    if (vErr > 0)
                    {
                        vResult = true;
                        ViewState["OccuId"] = vOcuId;
                    }
                    else
                    {
                ...
                else if (Mode == "Edit")
                {
                    oOcu = new COccu();
                    oGbl = new CGblIdGenerator();
                    vRec = oGbl.ChkDuplicate("OccupationMst", "Occupation", txtOccu.Text.Replace("'", "''"), "", "", "OccupationId", vOccuId, "Edit");
                    if (vRec > 0)
                    {
                        gblFuction.MsgPopup("Occupation Can not be Duplicate...");
                        return false;
                    }
                    vErr = oOcu.InsertOccu(ref vOcuId, txtOccu.Text.Replace("'", "''"), this.UserID, "E", "Edit");
                    if (vErr > 0)
                    {
                ...
                {
                    oGbl = new CGblIdGenerator();
                    vRec = oGbl.ChkDelete(vOcuId, "OccupationId", "MahallaSurvey");
                    if (vRec <= 0)
                    {
                        oOcu = new COccu();
                        vErr = oOcu.InsertOccu(ref vOcuId, txtOccu.Text.Replace("'", "''"), this.UserID, "D", "Del");
                        if (vErr > 0)
                        {
