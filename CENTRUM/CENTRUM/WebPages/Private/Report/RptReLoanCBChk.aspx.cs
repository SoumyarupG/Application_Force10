...
{
    public partial class RptReLoanCBChk : CENTRUMBase
    {
        ...
        private void InitBasePage()
        {
            ...
            {
                this.Menu = false;
                this.PageHeading = "Reloan CB Check Report";
                this.ShowBranchName = Session[gblValue.BrnchCode].ToString() + " - " + Session[gblValue.BrName].ToString();
                this.ShowFinYear = Session[gblValue.FinYear].ToString() + " ( Login Date " + Session[gblValue.LoginDate].ToString() + " )";
                this.GetModuleByRole(mnuID.mnuReloanCBChkRpt);
                ...
                {
                    Response.Redirect("~/WebPages/Public/PageAccess.aspx?mnuTxt=" + "Reloan CB Check Report", false);
                }
            }
            catch
            {
        ...
        private void SetParameterForRptData(string pMode)
        {
            ...
            try
            {
                ...
                oRpt = new CReports();
                dt = oRpt.RptReLoanCBChk(vBrCode, vFromDt, vToDt);
                if (dt.Rows.Count > 0)
                {
                    if (pMode == "Excel")
                    {
                        vFileNm = "attachment;filename=" + gblFuction.setDate(txtDtFrm.Text).ToString("yyyyMMdd") + "_ " + gblFuction.setDate(txtToDt.Text).ToString("yyyyMMdd") + "_Reloan_CB_Check.xls";
        ...
        private void PrintTxt(DataTable dt)
        {
            ...
            string vFileNm = "";
            vFileNm = vFolderPath + "\\" + gblFuction.setDate(txtDtFrm.Text).ToString("yyyyMMdd") + "_ " + gblFuction.setDate(txtToDt.Text).ToString("yyyyMMdd") + "_Reloan_CB_Check.txt";
