...
namespace CENTRUMBA
{
    public class CClaimeStatement
    {
        public Int32 SaveMemberDeathDocument(string pLoanid, int pDDSentBy, string pNomIdProofDoc, string pDeathCertificateDoc,
            ...
            string pNomPin,string pBankName,string pBranchName,string pAccountNo,string pIfsc,string pBenfishName)
        {
            Int32 vErr = 0;
        ...
        public DataTable GetDeathMemNomInfo(string pLoanId)
        {
            DataTable dt = new DataTable();
            SqlCommand oCmd = new SqlCommand();
            try
