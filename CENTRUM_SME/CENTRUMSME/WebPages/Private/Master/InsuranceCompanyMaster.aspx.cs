...
{
    public partial class InsuranceCompanyMaster : CENTRUMBAse
    {
        ...
        private void InitBasePage()
        {
            try
            {
                ...
                {
                    Response.Redirect("~/WebPages/Public/PageAccess.aspx?mnuTxt=" + "Insurance Company Master", false);
                }
            }
            catch
            {
        ...
        {
            DataTable dt = null;
            CInsureCompany oIC = null;
            Int32 vRows = 0;
            try
            {
                oIC = new CInsureCompany();
                dt = oIC.GetInsureCompanyPG(pPgIndx, ref vRows);
            ...
            try
            {
                ...
                if (e.CommandName == "cmdShow")
                {
                    ...
                    **/
                    ...
                    oIC = new CInsureCompany();
                    dt = oIC.GetInsureCompanyById(vICId);
                    if (dt.Rows.Count > 0)
                    {
                        txtCmpName.Text = Convert.ToString(dt.Rows[0]["ICName"]).Trim();
        ...
        private Boolean SaveRecords(string Mode)
        {
            ...
            string vSubId = Convert.ToString(ViewState["ICId"]);
            Int32 vErr = 0, vRec = 0, vICId = 0;
            CInsureCompany oIC = null;
            CGblIdGenerator oGbl = null;
            try
            {
                vICId = Convert.ToInt32(ViewState["ICId"]);
                if (Mode == "Save")
                {
                    oIC = new CInsureCompany();
                    oGbl = new CGblIdGenerator();
                    vRec = oGbl.ChkDuplicate("ICMst", "ICName", txtCmpName.Text.Replace("'", "''"), "", "", "ICId", vSubId, "Save");
                    if (vRec > 0)
                    {
                        gblFuction.MsgPopup("Company Name Can not be Duplicate...");
                        return false;
                    }
                    vErr = oIC.InsertInsuranceComp(ref vICId, txtCmpName.Text.Replace("'", "''"), txtCmpAdd.Text.Replace("'", "''"),
                        txtContPer1.Text.Replace("'", "''"), txtContNo1.Text.Replace("'", "''"), txtContPer2.Text.Replace("'", "''"),
                        txtContNo2.Text.Replace("'", "''"), this.UserID, "Save");
                    if (vErr > 0)
                    {
                        vResult = true;
                        ViewState["ICId"] = vICId;
                ...
                else if (Mode == "Edit")
                {
                    oIC = new CInsureCompany();
                    oGbl = new CGblIdGenerator();
                    vRec = oGbl.ChkDuplicate("ICMst", "ICName", txtCmpName.Text.Replace("'", "''"), "", "", "ICId", vSubId, "Edit");
                    if (vRec > 0)
                    {
                        gblFuction.MsgPopup("Company Name Can not be Duplicate...");
                        return false;
                    ...
                    vErr = oIC.InsertInsuranceComp(ref vICId, txtCmpName.Text.Replace("'", "''"), txtCmpAdd.Text.Replace("'", "''"),
                        txtContPer1.Text.Replace("'", "''"), txtContNo1.Text.Replace("'", "''"), txtContPer2.Text.Replace("'", "''"),
                        txtContNo2.Text.Replace("'", "''"), this.UserID, "Edit");
                    if (vErr > 0)
                    {
                ...
                {
                    oGbl = new CGblIdGenerator();
                    vRec = oGbl.ChkDelete(vICId, "ICId", "Insurance");
                    if (vRec <= 0)
                    {
                        oIC = new CInsureCompany();
                        vErr = oIC.InsertInsuranceComp(ref vICId, txtCmpName.Text.Replace("'", "''"), txtCmpAdd.Text.Replace("'", "''"),
                        txtContPer1.Text.Replace("'", "''"), txtContNo1.Text.Replace("'", "''"), txtContPer2.Text.Replace("'", "''"),
                        txtContNo2.Text.Replace("'", "''"), this.UserID, "Del");
                        if (vErr > 0)
                            vResult = true;
                        else
                        {
