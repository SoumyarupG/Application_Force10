...
{
    public partial class rptOTSMasterCheck : CENTRUMBase
    {
        ...
        private void InitBasePage()
        {
            ...
            {
                this.Menu = false;
                this.PageHeading = "OTS Master Check Report";
                this.ShowBranchName = Session[gblValue.BrnchCode].ToString() + " - " + Session[gblValue.BrName].ToString();
                this.ShowFinYear = Session[gblValue.FinYear].ToString() + " ( Login Date " + Session[gblValue.LoginDate].ToString() + " )";
                this.GetModuleByRole(mnuID.mnuOtsMasterReport);
                ...
                {
                    Response.Redirect("~/WebPages/Public/PageAccess.aspx?mnuTxt=" + "OTS Master Check Report", false);
                }
            }
            catch
            {
        ...
        protected void btnExcl_Click(object sender, EventArgs e)
        {
            ...
            if (vFinFrom > gblFuction.setDate(txtAsOnDt.Text) || gblFuction.setDate(txtAsOnDt.Text) > vFinTo)
            {
                gblFuction.MsgPopup("As On Date should be within this financial year.");
                return;
            }
            Int32 vRptOTSMasterCheckReport = Convert.ToInt32(Session[gblValue.RptOTSMasterCheckReport].ToString());
            if (vRptOTSMasterCheckReport != 0)
            {
                Int32 unixTicks = (Int32)(DateTime.UtcNow.Subtract(new DateTime(1970, 1, 1))).TotalSeconds;
                if (unixTicks - vRptOTSMasterCheckReport > 300)
                {
                    Session[gblValue.RptOTSMasterCheckReport] = ((Int32)(DateTime.UtcNow.Subtract(new DateTime(1970, 1, 1))).TotalSeconds).ToString();
                    GenerateRptData("Excel");
                }
                else
                {
        ...
        private void GenerateRptData(string pMode)
        {
            ...
            var req = new OTSMasterCheckReportRequest()
            {
            ...
            string vMsg = API.GenerateReport("GenerateOTSMasterCheckReport", Requestdata, vReportUrl);
    ...
    [DataContract]
    public class OTSMasterCheckReportRequest
    {
        [DataMember]
