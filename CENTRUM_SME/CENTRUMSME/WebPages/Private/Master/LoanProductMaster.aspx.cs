    ...
    public partial class LoanProductMaster : CENTRUMBAse
    {
        ...
        private void InitBasePage()
        {
            try
            {
                ...
                this.Menu = false;
                this.PageHeading = "Loan Product Master";
        ...
        private void StatusButton(String pMode)
        {
            switch (pMode)
            {
                case "Add":
                    ...
                    ClearControls();
                    gblFuction.focus("ctl00_cph_Main_tabLPMst_pnlDtl_txtProduct");
                    break;
                case "Show":
                    btnAdd.Enabled = true;
        ...
        private void LoadGrid(Int32 pPgIndx)
        {
            ...
            try
            {
                ...
                gvLoanPrdct.DataSource = dt.DefaultView;
                gvLoanPrdct.DataBind();
                if (dt.Rows.Count <= 0)
                {
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        protected void gvLoanPrdct_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            Int32 vProdctId = 0;
            DataTable dt = null;
            CProduct oPM = null;
            try
            {
                ...
                if (e.CommandName == "cmdShow")
                {
                    ...
                    /**
                    foreach (GridViewRow gr in gvLoanPrdct.Rows)
                    {
                    ...
                    **/
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        txtProduct.Text = Convert.ToString(dt.Rows[0]["ProductName"]).Trim();
                        txtShtName.Text = Convert.ToString(dt.Rows[0]["ShotName"]).Trim();
                        txtDate.Text = Convert.ToString(dt.Rows[0]["IntroDate"]);
                        lblUser.Text = "Last Modified By : " + dt.Rows[0]["UserName"].ToString();
                        lblDate.Text = "Last Modified Date : " + dt.Rows[0]["CreationDateTime"].ToString();
                        tabLPMst.ActiveTabIndex = 1;
                        StatusButton("Show");
                    }
                }
            }
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if ((vStateEdit == "Add" || vStateEdit == null) && gblFuction.IsDate(txtDate.Text))
            {
                vStateEdit = "Save";
            }
            else if ((vStateEdit == "Edit" || vStateEdit == null) && gblFuction.IsDate(txtDate.Text))
            {
                vStateEdit = "Edit";
            }
            else
            {
                gblFuction.MsgPopup(gblPRATAM.ValidDate);
                return;
            }
        ...
        private Boolean SaveRecords(string Mode)
        {
            ...
            //string vStateEdit = Convert.ToString(ViewState["StateEdit"]);
            string vSubId = Convert.ToString(ViewState["ProdctId"]);
            ...
            try
            {
                ...
                if (Mode == "Save")
                {
                    ...
                    vRec = oGbl.ChkDuplicate("ProductMst", "ProductName", txtProduct.Text.Replace("'", "''"), "", "", "ProductId", vSubId, "Save");
                    if (vRec > 0)
                    {
                        gblFuction.MsgPopup("Product Can not be Duplicate...");
                        return false;
                    }
                    vErr = oPM.InsertProduct(ref vProdctId, txtProduct.Text.Replace("'", "''"), txtShtName.Text.Replace("'", "''"), gblFuction.setDate(txtDate.Text), this.UserID, "I", "Save");
                    if (vErr > 0)
                    {
                        vResult = true;
                        ViewState["ProdctId"] = vProdctId;
                ...
                else if (Mode == "Edit")
                {
                    oPM = new CProduct();
                    oGbl = new CGblIdGenerator();
                    vRec = oGbl.ChkDuplicate("ProductMst", "ProductName", txtProduct.Text.Replace("'", "''"), "", "", "ProductId", vSubId, "Edit");
                    if (vRec > 0)
                    {
                        gblFuction.MsgPopup("Product Can not be Duplicate...");
                        return false;
                    }
                    vErr = oPM.InsertProduct(ref vProdctId, txtProduct.Text.Replace("'", "''"),txtShtName.Text.Replace("'", "''"),gblFuction.setDate(txtDate.Text),this.UserID, "E", "Edit");
                    if (vErr > 0)
                    {
                ...
                {
                    oGbl = new CGblIdGenerator();
                    vRec = oGbl.ChkDelete(vProdctId, "ProductId", "LoanTypeMst");
                    if (vRec <= 0)
                    {
                        oPM = new CProduct();
                        vErr = oPM.InsertProduct(ref vProdctId, txtProduct.Text.Replace("'", "''"),txtShtName.Text.Replace("'", "''"),gblFuction.setDate(txtDate.Text),this.UserID, "D", "Del");
                        if (vErr > 0)
                            vResult = true;
                        else
                        {
