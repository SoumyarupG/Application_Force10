...
{
    public partial class CF_ExternlCheck : CENTRUMBAse
    {
        ...
        private void InitBasePage()
        {
            try
            {
                ...
                this.Menu = false;
                this.PageHeading = "External Checks";
                this.ShowBranchName = Session[gblValue.BrnchCode].ToString() + " - " + Session[gblValue.BrName].ToString();
                this.ShowFinYear = Session[gblValue.FinYear].ToString() + " ( Login Date " + Session[gblValue.LoginDate].ToString() + " )";
                this.GetModuleByRole(mnuID.mnuCFExtrChk);
                ...
                {
                    Response.Redirect("~/WebPages/Public/PageAccess.aspx?mnuTxt=" + "External Checks", false);
                }
            }
            catch
            {
        ...
        private void ShowEmpRecords()
        {
            ...
            try
            {
                ...
                dt = oMem.CF_ExtrnlChkDtlByLeadId(Convert.ToInt64(vLoanAppId));
                if (dt.Rows.Count > 0)
                {
                    gvEmpDoc.DataSource = dt;
                    gvEmpDoc.DataBind();
        ...
        {
            string vTrdId = ddlTrdRfChk.SelectedValue.ToString();
            ...
            if (GetDoc() == true)
            {
                ...
                vErr = oMem.CF_SaveExtrnlChk(vLeadID, vTrdId, gblFuction.setDate(Session[gblValue.LoginDate].ToString()), vXmlData, DocumentBucketURL, Mode, this.UserID);
                if (vErr == 0)
                {
                    if (Mode == "Save")
                        gblFuction.MsgPopup(gblPRATAM.SaveMsg);
        ...
        #endregion        
        protected void GetExternalChecks()
        {
            Int64 pLeadId = 0;
            string vBrCode = "";
            ...
            ViewState["LeadId"] = pLeadId;
                    ...
                    dt = oDist.CF_GetExtrnlChkDtlLeadID(pLeadId);
                    if (dt.Rows.Count > 0)
                    {
                    ...
                    else
                    {
                        ...
                        txtLoanApplicationNo.Enabled = true;
                        if (Session[gblValue.BCPNO] != null)
                        {
                            txtLoanApplicationNo.Text = Convert.ToString(Session[gblValue.BCPNO]);
                        }
